...
access(all) contract CrappieCoin: FungibleToken {
    ...
    access(all) resource interface SpeciesCoinPublic {
        ...
        access(all) fun redeemCatchNFT(fishData: {String: AnyStruct}, angler: Address): @CrappieCoin.Vault
    }
        ...
        switch viewType {
            ...
            case Type<FungibleTokenMetadataViews.FTVaultData>():
                return FungibleTokenMetadataViews.FTVaultData(
                    ...
                    receiverLinkedType: Type<&CrappieCoin.Vault>(),
                    metadataLinkedType: Type<&CrappieCoin.Vault>(),
                    createEmptyVaultFunction: (fun(): @{FungibleToken.Vault} {
                        return <-CrappieCoin.createEmptyVault(vaultType: Type<@CrappieCoin.Vault>())
                    })
                )
    ...
    access(all) resource FishDEXCoordinator: SpeciesCoinPublic {
        ...
        access(all) view fun getSpeciesCode(): String {
            return CrappieCoin.speciesMetadata.speciesCode
        }
        
        access(all) view fun getTicker(): String {
            return CrappieCoin.speciesMetadata.ticker
        }
        
        access(all) view fun getCommonName(): String {
            return CrappieCoin.speciesMetadata.commonName
        }
        
        access(all) view fun getScientificName(): String {
            return CrappieCoin.speciesMetadata.scientificName
        }
        
        access(all) view fun getFamily(): String {
            return CrappieCoin.speciesMetadata.family
        }
        
        access(all) view fun getTotalSupply(): UFix64 {
            return CrappieCoin.totalSupply
        }
        ...
        access(all) view fun getBasicRegistryInfo(): {String: AnyStruct} {
            return {
                ...
                "contractAddress": CrappieCoin.account.address,
                "dataYear": CrappieCoin.speciesMetadata.dataYear,
                "conservationStatus": CrappieCoin.speciesMetadata.globalConservationStatus,
                "rarityTier": CrappieCoin.speciesMetadata.rarityTier,
                "isRegistered": CrappieCoin.isRegisteredWithFishDEX
            }
        }
        ...
        access(all) fun redeemCatchNFT(fishData: {String: AnyStruct}, angler: Address): @CrappieCoin.Vault {
            ...
            if let nftId = fishNFTId {
                assert(
                    CrappieCoin.fishNFTRegistry[nftId] == nil,
                    message: "This NFT has already been redeemed for a coin"
                )
                // Record this NFT as redeemed
                CrappieCoin.fishNFTRegistry[nftId] = true
            }
            
            // Auto-record first catch if this is the very first mint
            if CrappieCoin.totalSupply == 0.0 && CrappieCoin.speciesMetadata.firstCatchDate == nil {
                CrappieCoin.speciesMetadata.setFirstCatchDate(UInt64(getCurrentBlock().timestamp))
            ...
            CrappieCoin.totalSupply = CrappieCoin.totalSupply + amount
        ...
        access(all) fun registerWithFishDEX(fishDEXAddress: Address) {
            pre {
                !CrappieCoin.isRegisteredWithFishDEX: "Already registered with FishDEX"
            }
            ...
            CrappieCoin.fishDEXAddress = fishDEXAddress
            CrappieCoin.isRegisteredWithFishDEX = true
        ...
        access(all) fun updateFishDEXAddress(newAddress: Address) {
            let oldAddress = CrappieCoin.fishDEXAddress
            CrappieCoin.fishDEXAddress = newAddress
            CrappieCoin.isRegisteredWithFishDEX = false // Reset registration status
    ...
    access(all) resource Vault: FungibleToken.Vault {
        ...
        access(contract) fun burnCallback() {
            if self.balance > 0.0 {
                CrappieCoin.totalSupply = CrappieCoin.totalSupply - self.balance
        ...
        access(all) view fun getViews(): [Type] {
            return CrappieCoin.getContractViews(resourceType: nil)
        }

        access(all) fun resolveView(_ view: Type): AnyStruct? {
            return CrappieCoin.resolveContractView(resourceType: nil, viewType: view)
        }

        access(all) view fun getSupportedVaultTypes(): {Type: Bool} {
            return {Type<@CrappieCoin.Vault>(): true}
        }

        access(all) view fun isSupportedVaultType(type: Type): Bool {
            return type == Type<@CrappieCoin.Vault>()
        }
        ...
        access(FungibleToken.Withdraw) fun withdraw(amount: UFix64): @CrappieCoin.Vault {
            self.balance = self.balance - amount
            return <-create Vault(balance: amount)
        }

        access(all) fun deposit(from: @{FungibleToken.Vault}) {
            let vault <- from as! @CrappieCoin.Vault
            self.balance = self.balance + vault.balance
            vault.balance = 0.0
            destroy vault
        }

        access(all) fun createEmptyVault(): @CrappieCoin.Vault {
    ...
    access(all) resource Minter {
        ...
        access(all) fun mintForCatch(amount: UFix64, fishId: UInt64, angler: Address): @CrappieCoin.Vault {
            pre {
                amount == 1.0: "Only 1 coin per verified catch"
                CrappieCoin.fishNFTRegistry[fishId] == nil: "This NFT has already been used to mint a coin"
            }
            
            // Record this NFT as used
            CrappieCoin.fishNFTRegistry[fishId] = true
        ...
        access(all) fun autoRegisterWithFishDEX() {
            if let fishDEXAddr = CrappieCoin.fishDEXAddress {
                if !CrappieCoin.isRegisteredWithFishDEX {
                    // Get reference to FishDEX coordinator
                    if let coordinatorRef = CrappieCoin.account.capabilities.borrow<&FishDEXCoordinator>(
                        CrappieCoin.FishDEXCoordinatorPublicPath
                    ) {
        ...
        access(all) fun mintBatch(recipients: {Address: UFix64}): @{Address: CrappieCoin.Vault} {
            let vaults: @{Address: CrappieCoin.Vault} <- {}
    ...
    access(all) resource MetadataAdmin {
        ...
        access(all) fun updateImageURL(newURL: String) {
            let oldURL = CrappieCoin.speciesMetadata.imageURL ?? ""
            CrappieCoin.speciesMetadata.setImageURL(newURL)
        ...
        access(all) fun updateDescription(newDescription: String) {
            let oldDescription = CrappieCoin.speciesMetadata.description
            CrappieCoin.speciesMetadata.setDescription(newDescription)
        ...
        access(all) fun updateHabitat(newHabitat: String) {
            let oldHabitat = CrappieCoin.speciesMetadata.habitat ?? ""
            CrappieCoin.speciesMetadata.setHabitat(newHabitat)
        ...
        access(all) fun updateAverageWeight(newWeight: UFix64) {
            let oldWeight = CrappieCoin.speciesMetadata.averageWeight?.toString() ?? "0.0"
            CrappieCoin.speciesMetadata.setAverageWeight(newWeight)
        ...
        access(all) fun updateAverageLength(newLength: UFix64) {
            let oldLength = CrappieCoin.speciesMetadata.averageLength?.toString() ?? "0.0"
            CrappieCoin.speciesMetadata.setAverageLength(newLength)
        ...
        access(all) fun updateRarityTier(newTier: UInt8) {
            ...
            let oldTier = CrappieCoin.speciesMetadata.rarityTier?.toString() ?? "0"
            CrappieCoin.speciesMetadata.setRarityTier(newTier)
        ...
        access(all) fun manuallySetFirstCatch(timestamp: UInt64, angler: Address) {
            pre {
                CrappieCoin.speciesMetadata.firstCatchDate == nil: "First catch already recorded"
            }
            CrappieCoin.speciesMetadata.setFirstCatchDate(timestamp)
        ...
        access(all) fun updateConservationStatus(newStatus: String) {
            let oldStatus = CrappieCoin.speciesMetadata.globalConservationStatus ?? ""
            CrappieCoin.speciesMetadata.setConservationStatus(newStatus)
        ...
        access(all) fun updateSeasonalPatterns(newPatterns: String) {
            let oldPatterns = CrappieCoin.speciesMetadata.seasonalPatterns ?? ""
            CrappieCoin.speciesMetadata.setSeasonalPatterns(newPatterns)
        ...
        access(all) fun updateRecordWeight(newRecord: UFix64) {
            let oldRecord = CrappieCoin.speciesMetadata.recordWeight?.toString() ?? "0.0"
            CrappieCoin.speciesMetadata.setRecordWeight(newRecord)
        ...
        access(all) fun updateRecordLength(newRecord: UFix64) {
            let oldRecord = CrappieCoin.speciesMetadata.recordLength?.toString() ?? "0.0"
            CrappieCoin.speciesMetadata.setRecordLength(newRecord)
        ...
        access(all) fun updateRecordWeightLocation(newLocation: String?) {
            let oldLocation = CrappieCoin.speciesMetadata.recordWeightLocation ?? ""
            CrappieCoin.speciesMetadata.setRecordWeightLocation(newLocation)
        ...
        access(all) fun updateRecordWeightDate(newDate: String?) {
            let oldDate = CrappieCoin.speciesMetadata.recordWeightDate ?? ""
            CrappieCoin.speciesMetadata.setRecordWeightDate(newDate)
        ...
        access(all) fun updateRecordLengthLocation(newLocation: String?) {
            let oldLocation = CrappieCoin.speciesMetadata.recordLengthLocation ?? ""
            CrappieCoin.speciesMetadata.setRecordLengthLocation(newLocation)
        ...
        access(all) fun updateRecordLengthDate(newDate: String?) {
            let oldDate = CrappieCoin.speciesMetadata.recordLengthDate ?? ""
            CrappieCoin.speciesMetadata.setRecordLengthDate(newDate)
        ...
        access(all) fun approvePendingUpdate(index: Int) {
            pre {
                index >= 0 && index < CrappieCoin.pendingUpdates.length: "Invalid update index"
            }
            let update = CrappieCoin.pendingUpdates.remove(at: index)
        ...
        access(all) fun rejectPendingUpdate(index: Int) {
            ...
            CrappieCoin.pendingUpdates.remove(at: index)
        }
        
        access(all) fun clearAllPendingUpdates() {
            CrappieCoin.pendingUpdates = []
        }
        
        // ENHANCED ADMIN FUNCTIONS WITH VALIDATION
        access(all) fun updateRarityTierValidated(newTier: UInt8) {
            pre {
                CrappieCoin.validateRating(newTier): "Invalid rarity tier (must be 1-10)"
            }
            let oldTier = CrappieCoin.speciesMetadata.rarityTier?.toString() ?? "0"
            CrappieCoin.speciesMetadata.setRarityTier(newTier)
            emit MetadataUpdated(field: "rarityTier", oldValue: oldTier, newValue: newTier.toString())
        ...
        access(all) fun updateConservationStatusValidated(newStatus: String) {
            pre {
                CrappieCoin.validateConservationStatus(newStatus): "Invalid conservation status"
            }
            let oldStatus = CrappieCoin.speciesMetadata.globalConservationStatus ?? ""
            CrappieCoin.speciesMetadata.setConservationStatus(newStatus)
            emit MetadataUpdated(field: "globalConservationStatus", oldValue: oldStatus, newValue: newStatus)
        ...
        access(all) fun updateFightRatingValidated(newRating: UInt8) {
            pre {
                CrappieCoin.validateRating(newRating): "Fight rating must be 1-10"
            }
            let oldRating = CrappieCoin.speciesMetadata.fightRating?.toString() ?? "0"
            CrappieCoin.speciesMetadata.setFightRating(newRating)
        ...
        access(all) fun archiveCurrentYear() {
            let currentYear = CrappieCoin.currentMetadataYear
            CrappieCoin.metadataHistory[currentYear] = CrappieCoin.speciesMetadata
            emit YearlyMetadataCreated(year: currentYear)
        }
        
        access(all) fun updateToNewYear(_ newYear: UInt64) {
            pre {
                newYear > CrappieCoin.currentMetadataYear: "New year must be greater than current year"
            }
            // Archive current year data
            self.archiveCurrentYear()
            
            let oldYear = CrappieCoin.currentMetadataYear
            CrappieCoin.currentMetadataYear = newYear
            emit MetadataYearUpdated(oldYear: oldYear, newYear: newYear)
        }
    }

    // Public functions
    access(all) fun createEmptyVault(vaultType: Type): @CrappieCoin.Vault {
        pre {
            vaultType == Type<@CrappieCoin.Vault>(): "Vault type mismatch"
        }
        return <-create Vault(balance: 0.0)
    }
    ...
    // NOTE: Crappie coins represent permanent historical catch records
    ...
    access(all) view fun getVaultBalance(vaultRef: &CrappieCoin.Vault): UFix64 {
        return vaultRef.balance
    }
    
    access(all) view fun canWithdraw(vaultRef: &CrappieCoin.Vault, amount: UFix64): Bool {
    ...
    access(all) view fun isValidVaultType(vaultType: Type): Bool {
        return vaultType == Type<@CrappieCoin.Vault>()
    }
    ...
    init() {
        ...
        self.speciesMetadata = SpeciesMetadata(
            // IMMUTABLE CORE FIELDS
            speciesCode: "POMOXIS_NIGROMACULATUS",
            ticker: "POMNIG",
            ...
            // BASIC DESCRIPTIVE FIELDS
            commonName: "Crappie",
            habitat: "Freshwater lakes, reservoirs, slow-moving rivers, and ponds with vegetation",
            averageWeight: 1.5,
            averageLength: 12.0,
            imageURL: "https://derby.fish/images/species/Crappie.jpg",
            description: "Black Crappie are popular panfish known for their excellent table fare and schooling behavior. They have distinctive dark mottled patterns and are prized for their sweet, flaky meat.",
            firstCatchDate: nil,
            rarityTier: 2,
            ...
            regionalPopulations: {
                "Great Lakes": RegionalPopulation(
                    ...
                    threats: ["Habitat Loss", "Water Quality", "Invasive Species"],
                    protectedAreas: ["Great Lakes National Marine Sanctuaries"],
                    estimatedPopulation: 25000000
                ),
                "Southeast US": RegionalPopulation(
                    populationTrend: "Stable",
                    threats: ["Agricultural Runoff", "Dam Construction", "Sedimentation"],
                    protectedAreas: ["Tennessee River System", "Various State Parks"],
                    estimatedPopulation: nil
                ),
                "Central US": RegionalPopulation(
                    populationTrend: "Stable",
                    threats: ["Habitat Fragmentation", "Water Level Fluctuations", "Pollution"],
                    protectedAreas: ["Mark Twain National Forest Lakes"],
                    estimatedPopulation: nil
                )
            },
            ...
            // BIOLOGICAL INTELLIGENCE
            lifespan: 10.0,
            diet: "Zooplankton, insects, small fish, crustaceans, worms",
            predators: ["Largemouth Bass", "Northern Pike", "Walleye", "Muskellunge", "Humans"],
            temperatureRange: "50-85°F (optimal 68-78°F)",
            depthRange: "5-20 feet",
            spawningAge: 2.0,
            spawningBehavior: "Spawns in shallow areas with vegetation, males build nests and guard eggs",
            migrationPattern: "Moves to deeper water in winter, shallow areas during spawning",
            waterQualityNeeds: "pH 6.5-8.5, moderate dissolved oxygen, tolerates some turbidity",
            
            // GEOGRAPHIC & HABITAT
            nativeRegions: ["Eastern North America", "Great Lakes", "Mississippi River Basin"],
            currentRange: ["Throughout US", "Southern Canada", "Introduced to Western US"],
            waterTypes: ["Lake", "Reservoir", "River", "Pond", "Slow-moving Streams"],
            ...
            tourismValue: 7,
            ecosystemRole: "Secondary Consumer, Prey Species",
            culturalSignificance: "Popular family fishing target, important to recreational fishing culture",
            
            // ANGLING & RECREATIONAL
            bestBaits: ["Minnows", "Jigs", "Small spinners", "Worms", "Grubs"],
            fightRating: 6,
            culinaryRating: 9,
            catchDifficulty: 4,
            seasonalAvailability: "Best in spring and fall, active year-round in warm climates",
            bestTechniques: ["Vertical jigging", "Trolling", "Still fishing", "Spider rigging", "Dock fishing"],
            ...
            regionalRegulations: {
                ...
                "Texas": RegionalRegulations(
                    ...
                    bagLimit: 25,
                    closedSeasons: [],
                    specialRegulations: "No minimum on some waters",
                    licenseRequired: true
                )
            },
            
            // PHYSICAL & BEHAVIORAL
            physicalDescription: "Silver-green body with dark mottled patterns, deep compressed body, large fins",
            behaviorTraits: "Schooling fish, structure-oriented, active during dawn and dusk",
            seasonalPatterns: "Spring spawning in shallows, summer deep water, fall feeding aggregations",
            
            // RECORDS & ACHIEVEMENTS
            recordWeight: 5.0,
            recordWeightLocation: "Privateer Lake, Louisiana",
            recordWeightDate: "March 15, 2006",
            recordLength: 19.3,
            recordLengthLocation: "Weiss Lake, Alabama",
            recordLengthDate: "April 21, 2018",
            
            // RESEARCH & SCIENTIFIC
            researchPriority: 6,
            geneticMarkers: "Studies on stock enhancement and population genetics",
            studyPrograms: ["State fisheries agencies", "University research programs", "Regional fisheries councils"],
        ...
        self.VaultStoragePath = /storage/CrappieCoinVault
        self.VaultPublicPath = /public/CrappieCoinReceiver
        self.MinterStoragePath = /storage/CrappieCoinMinter
        self.MetadataAdminStoragePath = /storage/CrappieCoinMetadataAdmin
        self.FishDEXCoordinatorStoragePath = /storage/CrappieCoinFishDEXCoordinator
        self.FishDEXCoordinatorPublicPath = /public/CrappieCoinFishDEXCoordinator
        ...
        let cap = self.account.capabilities.storage.issue<&CrappieCoin.Vault>(self.VaultStoragePath)
