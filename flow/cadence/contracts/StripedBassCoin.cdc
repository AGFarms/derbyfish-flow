...
access(all) contract StripedBassCoin: FungibleToken {
    ...
    access(all) resource interface SpeciesCoinPublic {
        ...
        access(all) fun redeemCatchNFT(fishData: {String: AnyStruct}, angler: Address): @StripedBassCoin.Vault
    }
        ...
        switch viewType {
            ...
            case Type<FungibleTokenMetadataViews.FTVaultData>():
                return FungibleTokenMetadataViews.FTVaultData(
                    ...
                    receiverLinkedType: Type<&StripedBassCoin.Vault>(),
                    metadataLinkedType: Type<&StripedBassCoin.Vault>(),
                    createEmptyVaultFunction: (fun(): @{FungibleToken.Vault} {
                        return <-StripedBassCoin.createEmptyVault(vaultType: Type<@StripedBassCoin.Vault>())
                    })
                )
    ...
    access(all) resource FishDEXCoordinator: SpeciesCoinPublic {
        ...
        access(all) view fun getSpeciesCode(): String {
            return StripedBassCoin.speciesMetadata.speciesCode
        }
        
        access(all) view fun getTicker(): String {
            return StripedBassCoin.speciesMetadata.ticker
        }
        
        access(all) view fun getCommonName(): String {
            return StripedBassCoin.speciesMetadata.commonName
        }
        
        access(all) view fun getScientificName(): String {
            return StripedBassCoin.speciesMetadata.scientificName
        }
        
        access(all) view fun getFamily(): String {
            return StripedBassCoin.speciesMetadata.family
        }
        
        access(all) view fun getTotalSupply(): UFix64 {
            return StripedBassCoin.totalSupply
        }
        ...
        access(all) view fun getBasicRegistryInfo(): {String: AnyStruct} {
            return {
                ...
                "contractAddress": StripedBassCoin.account.address,
                "dataYear": StripedBassCoin.speciesMetadata.dataYear,
                "conservationStatus": StripedBassCoin.speciesMetadata.globalConservationStatus,
                "rarityTier": StripedBassCoin.speciesMetadata.rarityTier,
                "isRegistered": StripedBassCoin.isRegisteredWithFishDEX
            }
        }
        ...
        access(all) fun redeemCatchNFT(fishData: {String: AnyStruct}, angler: Address): @StripedBassCoin.Vault {
            ...
            if let nftId = fishNFTId {
                assert(
                    StripedBassCoin.fishNFTRegistry[nftId] == nil,
                    message: "This NFT has already been redeemed for a coin"
                )
                // Record this NFT as redeemed
                StripedBassCoin.fishNFTRegistry[nftId] = true
            }
            
            // Auto-record first catch if this is the very first mint
            if StripedBassCoin.totalSupply == 0.0 && StripedBassCoin.speciesMetadata.firstCatchDate == nil {
                StripedBassCoin.speciesMetadata.setFirstCatchDate(UInt64(getCurrentBlock().timestamp))
            ...
            StripedBassCoin.totalSupply = StripedBassCoin.totalSupply + amount
        ...
        access(all) fun registerWithFishDEX(fishDEXAddress: Address) {
            pre {
                !StripedBassCoin.isRegisteredWithFishDEX: "Already registered with FishDEX"
            }
            ...
            StripedBassCoin.fishDEXAddress = fishDEXAddress
            StripedBassCoin.isRegisteredWithFishDEX = true
        ...
        access(all) fun updateFishDEXAddress(newAddress: Address) {
            let oldAddress = StripedBassCoin.fishDEXAddress
            StripedBassCoin.fishDEXAddress = newAddress
            StripedBassCoin.isRegisteredWithFishDEX = false // Reset registration status
    ...
    access(all) resource Vault: FungibleToken.Vault {
        ...
        access(contract) fun burnCallback() {
            if self.balance > 0.0 {
                StripedBassCoin.totalSupply = StripedBassCoin.totalSupply - self.balance
        ...
        access(all) view fun getViews(): [Type] {
            return StripedBassCoin.getContractViews(resourceType: nil)
        }

        access(all) fun resolveView(_ view: Type): AnyStruct? {
            return StripedBassCoin.resolveContractView(resourceType: nil, viewType: view)
        }

        access(all) view fun getSupportedVaultTypes(): {Type: Bool} {
            return {Type<@StripedBassCoin.Vault>(): true}
        }

        access(all) view fun isSupportedVaultType(type: Type): Bool {
            return type == Type<@StripedBassCoin.Vault>()
        }
        ...
        access(FungibleToken.Withdraw) fun withdraw(amount: UFix64): @StripedBassCoin.Vault {
            self.balance = self.balance - amount
            return <-create Vault(balance: amount)
        }

        access(all) fun deposit(from: @{FungibleToken.Vault}) {
            let vault <- from as! @StripedBassCoin.Vault
            self.balance = self.balance + vault.balance
            vault.balance = 0.0
            destroy vault
        }

        access(all) fun createEmptyVault(): @StripedBassCoin.Vault {
    ...
    access(all) resource Minter {
        ...
        access(all) fun mintForCatch(amount: UFix64, fishId: UInt64, angler: Address): @StripedBassCoin.Vault {
            pre {
                amount == 1.0: "Only 1 coin per verified catch"
                StripedBassCoin.fishNFTRegistry[fishId] == nil: "This NFT has already been used to mint a coin"
            }
            
            // Record this NFT as used
            StripedBassCoin.fishNFTRegistry[fishId] = true
        ...
        access(all) fun autoRegisterWithFishDEX() {
            if let fishDEXAddr = StripedBassCoin.fishDEXAddress {
                if !StripedBassCoin.isRegisteredWithFishDEX {
                    // Get reference to FishDEX coordinator
                    if let coordinatorRef = StripedBassCoin.account.capabilities.borrow<&FishDEXCoordinator>(
                        StripedBassCoin.FishDEXCoordinatorPublicPath
                    ) {
        ...
        access(all) fun mintBatch(recipients: {Address: UFix64}): @{Address: StripedBassCoin.Vault} {
            let vaults: @{Address: StripedBassCoin.Vault} <- {}
    ...
    access(all) resource MetadataAdmin {
        ...
        access(all) fun updateImageURL(newURL: String) {
            let oldURL = StripedBassCoin.speciesMetadata.imageURL ?? ""
            StripedBassCoin.speciesMetadata.setImageURL(newURL)
        ...
        access(all) fun updateDescription(newDescription: String) {
            let oldDescription = StripedBassCoin.speciesMetadata.description
            StripedBassCoin.speciesMetadata.setDescription(newDescription)
        ...
        access(all) fun updateHabitat(newHabitat: String) {
            let oldHabitat = StripedBassCoin.speciesMetadata.habitat ?? ""
            StripedBassCoin.speciesMetadata.setHabitat(newHabitat)
        ...
        access(all) fun updateAverageWeight(newWeight: UFix64) {
            let oldWeight = StripedBassCoin.speciesMetadata.averageWeight?.toString() ?? "0.0"
            StripedBassCoin.speciesMetadata.setAverageWeight(newWeight)
        ...
        access(all) fun updateAverageLength(newLength: UFix64) {
            let oldLength = StripedBassCoin.speciesMetadata.averageLength?.toString() ?? "0.0"
            StripedBassCoin.speciesMetadata.setAverageLength(newLength)
        ...
        access(all) fun updateRarityTier(newTier: UInt8) {
            ...
            let oldTier = StripedBassCoin.speciesMetadata.rarityTier?.toString() ?? "0"
            StripedBassCoin.speciesMetadata.setRarityTier(newTier)
        ...
        access(all) fun manuallySetFirstCatch(timestamp: UInt64, angler: Address) {
            pre {
                StripedBassCoin.speciesMetadata.firstCatchDate == nil: "First catch already recorded"
            }
            StripedBassCoin.speciesMetadata.setFirstCatchDate(timestamp)
        ...
        access(all) fun updateConservationStatus(newStatus: String) {
            let oldStatus = StripedBassCoin.speciesMetadata.globalConservationStatus ?? ""
            StripedBassCoin.speciesMetadata.setConservationStatus(newStatus)
        ...
        access(all) fun updateNativeRegions(newRegions: [String]) {
            var oldRegionsStr = "["
            for i, region in StripedBassCoin.speciesMetadata.nativeRegions {
                if i > 0 { oldRegionsStr = oldRegionsStr.concat(",") }
                oldRegionsStr = oldRegionsStr.concat(region)
            }
            oldRegionsStr = oldRegionsStr.concat("]")
            
            StripedBassCoin.speciesMetadata.setNativeRegions(newRegions)
        ...
        access(all) fun updateSeasonalPatterns(newPatterns: String) {
            let oldPatterns = StripedBassCoin.speciesMetadata.seasonalPatterns ?? ""
            StripedBassCoin.speciesMetadata.setSeasonalPatterns(newPatterns)
        ...
        access(all) fun updateRecordWeight(newRecord: UFix64) {
            let oldRecord = StripedBassCoin.speciesMetadata.recordWeight?.toString() ?? "0.0"
            StripedBassCoin.speciesMetadata.setRecordWeight(newRecord)
        ...
        access(all) fun updateRecordLength(newRecord: UFix64) {
            let oldRecord = StripedBassCoin.speciesMetadata.recordLength?.toString() ?? "0.0"
            StripedBassCoin.speciesMetadata.setRecordLength(newRecord)
        ...
        access(all) fun updateRecordWeightLocation(newLocation: String?) {
            let oldLocation = StripedBassCoin.speciesMetadata.recordWeightLocation ?? ""
            StripedBassCoin.speciesMetadata.setRecordWeightLocation(newLocation)
        ...
        access(all) fun updateRecordWeightDate(newDate: String?) {
            let oldDate = StripedBassCoin.speciesMetadata.recordWeightDate ?? ""
            StripedBassCoin.speciesMetadata.setRecordWeightDate(newDate)
        ...
        access(all) fun updateRecordLengthLocation(newLocation: String?) {
            let oldLocation = StripedBassCoin.speciesMetadata.recordLengthLocation ?? ""
            StripedBassCoin.speciesMetadata.setRecordLengthLocation(newLocation)
        ...
        access(all) fun updateRecordLengthDate(newDate: String?) {
            let oldDate = StripedBassCoin.speciesMetadata.recordLengthDate ?? ""
            StripedBassCoin.speciesMetadata.setRecordLengthDate(newDate)
        ...
        access(all) fun approvePendingUpdate(index: Int) {
            pre {
                index >= 0 && index < StripedBassCoin.pendingUpdates.length: "Invalid update index"
            }
            let update = StripedBassCoin.pendingUpdates.remove(at: index)
        ...
        access(all) fun rejectPendingUpdate(index: Int) {
            ...
            StripedBassCoin.pendingUpdates.remove(at: index)
        }
        
        access(all) fun clearAllPendingUpdates() {
            StripedBassCoin.pendingUpdates = []
        }
        
        // ENHANCED ADMIN FUNCTIONS WITH VALIDATION
        access(all) fun updateRarityTierValidated(newTier: UInt8) {
            pre {
                StripedBassCoin.validateRating(newTier): "Invalid rarity tier (must be 1-10)"
            }
            let oldTier = StripedBassCoin.speciesMetadata.rarityTier?.toString() ?? "0"
            StripedBassCoin.speciesMetadata.setRarityTier(newTier)
            emit MetadataUpdated(field: "rarityTier", oldValue: oldTier, newValue: newTier.toString())
        ...
        access(all) fun updateConservationStatusValidated(newStatus: String) {
            pre {
                StripedBassCoin.validateConservationStatus(newStatus): "Invalid conservation status"
            }
            let oldStatus = StripedBassCoin.speciesMetadata.globalConservationStatus ?? ""
            StripedBassCoin.speciesMetadata.setConservationStatus(newStatus)
            emit MetadataUpdated(field: "globalConservationStatus", oldValue: oldStatus, newValue: newStatus)
        ...
        access(all) fun updateFightRatingValidated(newRating: UInt8) {
            pre {
                StripedBassCoin.validateRating(newRating): "Fight rating must be 1-10"
            }
            let oldRating = StripedBassCoin.speciesMetadata.fightRating?.toString() ?? "0"
            StripedBassCoin.speciesMetadata.setFightRating(newRating)
        ...
        access(all) fun archiveCurrentYear() {
            let currentYear = StripedBassCoin.currentMetadataYear
            StripedBassCoin.metadataHistory[currentYear] = StripedBassCoin.speciesMetadata
            emit YearlyMetadataCreated(year: currentYear)
        }
        
        access(all) fun updateToNewYear(_ newYear: UInt64) {
            pre {
                newYear > StripedBassCoin.currentMetadataYear: "New year must be greater than current year"
            }
            // Archive current year data
            self.archiveCurrentYear()
            
            let oldYear = StripedBassCoin.currentMetadataYear
            StripedBassCoin.currentMetadataYear = newYear
            emit MetadataYearUpdated(oldYear: oldYear, newYear: newYear)
        }
    }

    // Public functions
    access(all) fun createEmptyVault(vaultType: Type): @StripedBassCoin.Vault {
        pre {
            vaultType == Type<@StripedBassCoin.Vault>(): "Vault type mismatch"
        }
        return <-create Vault(balance: 0.0)
    }
    ...
    // NOTE: StripedBass coins represent permanent historical catch records
    ...
    access(all) view fun getVaultBalance(vaultRef: &StripedBassCoin.Vault): UFix64 {
        return vaultRef.balance
    }
    
    access(all) view fun canWithdraw(vaultRef: &StripedBassCoin.Vault, amount: UFix64): Bool {
    ...
    access(all) view fun isValidVaultType(vaultType: Type): Bool {
        return vaultType == Type<@StripedBassCoin.Vault>()
    }
    ...
    init() {
        ...
        self.speciesMetadata = SpeciesMetadata(
            ...
            habitat: "Coastal waters, estuaries, rivers, landlocked reservoirs",
            averageWeight: 18.0,
            averageLength: 28.0,
            imageURL: "https://derby.fish/images/species/StripedBass.jpg",
            description: "Striped Bass are iconic anadromous fish known for their distinctive horizontal black stripes and powerful fighting ability. They are highly prized by both commercial and recreational fishermen, and have been successfully introduced to many freshwater systems.",
            firstCatchDate: nil,
            rarityTier: 3,
            ...
            regionalPopulations: {
                "Atlantic Coast": RegionalPopulation(
                    populationTrend: "Stable",
                    threats: ["Overfishing", "Habitat Loss", "Climate Change"],
                    protectedAreas: ["Chesapeake Bay", "Delaware Bay", "Marine Protected Areas"],
                    estimatedPopulation: nil
                ),
                "Pacific Coast": RegionalPopulation(
                    populationTrend: "Stable",
                    threats: ["Water Diversions", "Habitat Modification", "Climate Change"],
                    protectedAreas: ["San Francisco Bay Delta", "Central Valley Waters"],
                    estimatedPopulation: nil
                ),
                "Inland Waters": RegionalPopulation(
                    populationTrend: "Stable",
                    threats: ["Water Temperature", "Habitat Changes", "Overfishing"],
                    protectedAreas: ["Various State Parks", "Federal Reservoirs"],
                    estimatedPopulation: nil
                )
            },
            ...
            // BIOLOGICAL INTELLIGENCE
            lifespan: 30.0,
            diet: "Herring, anchovies, sardines, shad, squid, crabs, worms",
            predators: ["Sharks", "Bluefish", "Seals", "Birds", "Humans"],
            temperatureRange: "50-75°F (optimal 60-70°F)",
            depthRange: "0-120 feet",
            spawningAge: 4.0,
            spawningBehavior: "Anadromous, spawns in freshwater rivers, releases semi-buoyant eggs",
            migrationPattern: "Coastal migrations following baitfish, seasonal movements between fresh and salt water",
            waterQualityNeeds: "Clean water, good dissolved oxygen, can tolerate varying salinity",
            
            // GEOGRAPHIC & HABITAT
            nativeRegions: ["Atlantic Coast", "Gulf of St. Lawrence"],
            currentRange: ["Atlantic Coast", "Pacific Coast", "Inland Lakes", "Reservoirs"],
            waterTypes: ["Coastal", "Estuarine", "River", "Reservoir", "Lake"],
            invasiveStatus: "Native to Atlantic, Introduced elsewhere",
            
            // ECONOMIC & COMMERCIAL
            regionalCommercialValue: {
                "Chesapeake Bay": 14.00,
                "Atlantic Coast": 12.00,
                "Pacific Coast": 16.00,
                "Inland Waters": 10.00
            },
            tourismValue: 9,
            ecosystemRole: "Apex Predator, Key Migratory Species",
            culturalSignificance: "State fish of Maryland, Rhode Island, and South Carolina, historic importance to Native Americans",
            
            // ANGLING & RECREATIONAL
            bestBaits: ["Live eels", "Bunker", "Clams", "Worms", "Lures"],
            fightRating: 8,
            culinaryRating: 9,
            catchDifficulty: 6,
            seasonalAvailability: "Spring and fall migrations, year-round in some areas",
            bestTechniques: ["Trolling", "Live bait fishing", "Surf fishing", "Casting lures", "Jigging"],
            
            // REGULATORY
            regionalRegulations: {
                "Atlantic Coast": RegionalRegulations(
                    sizeLimit: 28.0,
                    bagLimit: 1,
                    closedSeasons: [],
                    specialRegulations: "Slot limits in some areas, circle hooks required",
                    licenseRequired: true
                ),
                "Chesapeake Bay": RegionalRegulations(
                    sizeLimit: 19.0,
                    bagLimit: 1,
                    closedSeasons: [],
                    specialRegulations: "Seasonal closures, trophy seasons",
                    licenseRequired: true
                ),
                "Pacific Coast": RegionalRegulations(
                    sizeLimit: 18.0,
                    bagLimit: 2,
                    closedSeasons: [],
                    specialRegulations: "Varies by water body, some areas closed",
                    licenseRequired: true
                )
            },
            
            // PHYSICAL & BEHAVIORAL
            physicalDescription: "Silver body with 6-9 dark horizontal stripes, white belly, two dorsal fins, forked tail",
            behaviorTraits: "Schooling behavior, surface and mid-water feeding, highly migratory",
            seasonalPatterns: "Spring spawning runs, summer feeding, fall migrations, winter deep water",
            
            // RECORDS & ACHIEVEMENTS
            recordWeight: 81.0,
            recordWeightLocation: "Atlantic City, New Jersey",
            recordWeightDate: "September 21, 1982",
            recordLength: 54.0,
            recordLengthLocation: "Chesapeake Bay, Maryland",
            recordLengthDate: "May 15, 1995",
            
            // RESEARCH & SCIENTIFIC
            researchPriority: 8,
            geneticMarkers: "Extensive tagging studies and genetic analysis for stock assessment",
            studyPrograms: ["Atlantic States Marine Fisheries Commission", "NOAA Fisheries", "State Fish and Wildlife Agencies"],
            ...
            additionalMetadata: {
                ...
                "state_fish": "Maryland, Rhode Island, South Carolina",
                "nickname": "Striper"
            }
        )

        // Set storage paths using common name format
        self.VaultStoragePath = /storage/StripedBassCoinVault
        self.VaultPublicPath = /public/StripedBassCoinReceiver
        self.MinterStoragePath = /storage/StripedBassCoinMinter
        self.MetadataAdminStoragePath = /storage/StripedBassCoinMetadataAdmin
        self.FishDEXCoordinatorStoragePath = /storage/StripedBassCoinFishDEXCoordinator
        self.FishDEXCoordinatorPublicPath = /public/StripedBassCoinFishDEXCoordinator
        ...
        let cap = self.account.capabilities.storage.issue<&StripedBassCoin.Vault>(self.VaultStoragePath)
