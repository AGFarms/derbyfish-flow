...
access(all) contract SpottedBassCoin: FungibleToken {
    ...
    access(all) resource interface SpeciesCoinPublic {
        ...
        access(all) fun redeemCatchNFT(fishData: {String: AnyStruct}, angler: Address): @SpottedBassCoin.Vault
    }
        ...
        switch viewType {
            ...
            case Type<FungibleTokenMetadataViews.FTVaultData>():
                return FungibleTokenMetadataViews.FTVaultData(
                    ...
                    receiverLinkedType: Type<&SpottedBassCoin.Vault>(),
                    metadataLinkedType: Type<&SpottedBassCoin.Vault>(),
                    createEmptyVaultFunction: (fun(): @{FungibleToken.Vault} {
                        return <-SpottedBassCoin.createEmptyVault(vaultType: Type<@SpottedBassCoin.Vault>())
                    })
                )
    ...
    access(all) resource FishDEXCoordinator: SpeciesCoinPublic {
        ...
        access(all) view fun getSpeciesCode(): String {
            return SpottedBassCoin.speciesMetadata.speciesCode
        }
        
        access(all) view fun getTicker(): String {
            return SpottedBassCoin.speciesMetadata.ticker
        }
        
        access(all) view fun getCommonName(): String {
            return SpottedBassCoin.speciesMetadata.commonName
        }
        
        access(all) view fun getScientificName(): String {
            return SpottedBassCoin.speciesMetadata.scientificName
        }
        
        access(all) view fun getFamily(): String {
            return SpottedBassCoin.speciesMetadata.family
        }
        
        access(all) view fun getTotalSupply(): UFix64 {
            return SpottedBassCoin.totalSupply
        }
        ...
        access(all) view fun getBasicRegistryInfo(): {String: AnyStruct} {
            return {
                ...
                "contractAddress": SpottedBassCoin.account.address,
                "dataYear": SpottedBassCoin.speciesMetadata.dataYear,
                "conservationStatus": SpottedBassCoin.speciesMetadata.globalConservationStatus,
                "rarityTier": SpottedBassCoin.speciesMetadata.rarityTier,
                "isRegistered": SpottedBassCoin.isRegisteredWithFishDEX
            }
        }
        ...
        access(all) fun redeemCatchNFT(fishData: {String: AnyStruct}, angler: Address): @SpottedBassCoin.Vault {
            ...
            if let nftId = fishNFTId {
                assert(
                    SpottedBassCoin.fishNFTRegistry[nftId] == nil,
                    message: "This NFT has already been redeemed for a coin"
                )
                // Record this NFT as redeemed
                SpottedBassCoin.fishNFTRegistry[nftId] = true
            }
            
            // Auto-record first catch if this is the very first mint
            if SpottedBassCoin.totalSupply == 0.0 && SpottedBassCoin.speciesMetadata.firstCatchDate == nil {
                SpottedBassCoin.speciesMetadata.setFirstCatchDate(UInt64(getCurrentBlock().timestamp))
            ...
            SpottedBassCoin.totalSupply = SpottedBassCoin.totalSupply + amount
        ...
        access(all) fun registerWithFishDEX(fishDEXAddress: Address) {
            pre {
                !SpottedBassCoin.isRegisteredWithFishDEX: "Already registered with FishDEX"
            }
            ...
            SpottedBassCoin.fishDEXAddress = fishDEXAddress
            SpottedBassCoin.isRegisteredWithFishDEX = true
        ...
        access(all) fun updateFishDEXAddress(newAddress: Address) {
            let oldAddress = SpottedBassCoin.fishDEXAddress
            SpottedBassCoin.fishDEXAddress = newAddress
            SpottedBassCoin.isRegisteredWithFishDEX = false // Reset registration status
    ...
    access(all) resource Vault: FungibleToken.Vault {
        ...
        access(contract) fun burnCallback() {
            if self.balance > 0.0 {
                SpottedBassCoin.totalSupply = SpottedBassCoin.totalSupply - self.balance
        ...
        access(all) view fun getViews(): [Type] {
            return SpottedBassCoin.getContractViews(resourceType: nil)
        }

        access(all) fun resolveView(_ view: Type): AnyStruct? {
            return SpottedBassCoin.resolveContractView(resourceType: nil, viewType: view)
        }

        access(all) view fun getSupportedVaultTypes(): {Type: Bool} {
            return {Type<@SpottedBassCoin.Vault>(): true}
        }

        access(all) view fun isSupportedVaultType(type: Type): Bool {
            return type == Type<@SpottedBassCoin.Vault>()
        }
        ...
        access(FungibleToken.Withdraw) fun withdraw(amount: UFix64): @SpottedBassCoin.Vault {
            self.balance = self.balance - amount
            return <-create Vault(balance: amount)
        }

        access(all) fun deposit(from: @{FungibleToken.Vault}) {
            let vault <- from as! @SpottedBassCoin.Vault
            self.balance = self.balance + vault.balance
            vault.balance = 0.0
            destroy vault
        }

        access(all) fun createEmptyVault(): @SpottedBassCoin.Vault {
    ...
    access(all) resource Minter {
        ...
        access(all) fun mintForCatch(amount: UFix64, fishId: UInt64, angler: Address): @SpottedBassCoin.Vault {
            pre {
                amount == 1.0: "Only 1 coin per verified catch"
                SpottedBassCoin.fishNFTRegistry[fishId] == nil: "This NFT has already been used to mint a coin"
            }
            
            // Record this NFT as used
            SpottedBassCoin.fishNFTRegistry[fishId] = true
        ...
        access(all) fun autoRegisterWithFishDEX() {
            if let fishDEXAddr = SpottedBassCoin.fishDEXAddress {
                if !SpottedBassCoin.isRegisteredWithFishDEX {
                    // Get reference to FishDEX coordinator
                    if let coordinatorRef = SpottedBassCoin.account.capabilities.borrow<&FishDEXCoordinator>(
                        SpottedBassCoin.FishDEXCoordinatorPublicPath
                    ) {
        ...
        access(all) fun mintBatch(recipients: {Address: UFix64}): @{Address: SpottedBassCoin.Vault} {
            let vaults: @{Address: SpottedBassCoin.Vault} <- {}
    ...
    access(all) resource MetadataAdmin {
        ...
        access(all) fun updateImageURL(newURL: String) {
            let oldURL = SpottedBassCoin.speciesMetadata.imageURL ?? ""
            SpottedBassCoin.speciesMetadata.setImageURL(newURL)
        ...
        access(all) fun updateDescription(newDescription: String) {
            let oldDescription = SpottedBassCoin.speciesMetadata.description
            SpottedBassCoin.speciesMetadata.setDescription(newDescription)
        ...
        access(all) fun updateHabitat(newHabitat: String) {
            let oldHabitat = SpottedBassCoin.speciesMetadata.habitat ?? ""
            SpottedBassCoin.speciesMetadata.setHabitat(newHabitat)
        ...
        access(all) fun updateAverageWeight(newWeight: UFix64) {
            let oldWeight = SpottedBassCoin.speciesMetadata.averageWeight?.toString() ?? "0.0"
            SpottedBassCoin.speciesMetadata.setAverageWeight(newWeight)
        ...
        access(all) fun updateAverageLength(newLength: UFix64) {
            let oldLength = SpottedBassCoin.speciesMetadata.averageLength?.toString() ?? "0.0"
            SpottedBassCoin.speciesMetadata.setAverageLength(newLength)
        ...
        access(all) fun updateRarityTier(newTier: UInt8) {
            ...
            let oldTier = SpottedBassCoin.speciesMetadata.rarityTier?.toString() ?? "0"
            SpottedBassCoin.speciesMetadata.setRarityTier(newTier)
        ...
        access(all) fun manuallySetFirstCatch(timestamp: UInt64, angler: Address) {
            pre {
                SpottedBassCoin.speciesMetadata.firstCatchDate == nil: "First catch already recorded"
            }
            SpottedBassCoin.speciesMetadata.setFirstCatchDate(timestamp)
        ...
        access(all) fun updateConservationStatus(newStatus: String) {
            let oldStatus = SpottedBassCoin.speciesMetadata.globalConservationStatus ?? ""
            SpottedBassCoin.speciesMetadata.setConservationStatus(newStatus)
        ...
        access(all) fun updateNativeRegions(newRegions: [String]) {
            var oldRegionsStr = "["
            for i, region in SpottedBassCoin.speciesMetadata.nativeRegions {
                if i > 0 { oldRegionsStr = oldRegionsStr.concat(",") }
                oldRegionsStr = oldRegionsStr.concat(region)
            }
            oldRegionsStr = oldRegionsStr.concat("]")
            
            SpottedBassCoin.speciesMetadata.setNativeRegions(newRegions)
        ...
        access(all) fun updateSeasonalPatterns(newPatterns: String) {
            let oldPatterns = SpottedBassCoin.speciesMetadata.seasonalPatterns ?? ""
            SpottedBassCoin.speciesMetadata.setSeasonalPatterns(newPatterns)
        ...
        access(all) fun updateRecordWeight(newRecord: UFix64) {
            let oldRecord = SpottedBassCoin.speciesMetadata.recordWeight?.toString() ?? "0.0"
            SpottedBassCoin.speciesMetadata.setRecordWeight(newRecord)
        ...
        access(all) fun updateRecordLength(newRecord: UFix64) {
            let oldRecord = SpottedBassCoin.speciesMetadata.recordLength?.toString() ?? "0.0"
            SpottedBassCoin.speciesMetadata.setRecordLength(newRecord)
        ...
        access(all) fun updateRecordWeightLocation(newLocation: String?) {
            let oldLocation = SpottedBassCoin.speciesMetadata.recordWeightLocation ?? ""
            SpottedBassCoin.speciesMetadata.setRecordWeightLocation(newLocation)
        ...
        access(all) fun updateRecordWeightDate(newDate: String?) {
            let oldDate = SpottedBassCoin.speciesMetadata.recordWeightDate ?? ""
            SpottedBassCoin.speciesMetadata.setRecordWeightDate(newDate)
        ...
        access(all) fun updateRecordLengthLocation(newLocation: String?) {
            let oldLocation = SpottedBassCoin.speciesMetadata.recordLengthLocation ?? ""
            SpottedBassCoin.speciesMetadata.setRecordLengthLocation(newLocation)
        ...
        access(all) fun updateRecordLengthDate(newDate: String?) {
            let oldDate = SpottedBassCoin.speciesMetadata.recordLengthDate ?? ""
            SpottedBassCoin.speciesMetadata.setRecordLengthDate(newDate)
        ...
        access(all) fun approvePendingUpdate(index: Int) {
            pre {
                index >= 0 && index < SpottedBassCoin.pendingUpdates.length: "Invalid update index"
            }
            let update = SpottedBassCoin.pendingUpdates.remove(at: index)
        ...
        access(all) fun rejectPendingUpdate(index: Int) {
            ...
            SpottedBassCoin.pendingUpdates.remove(at: index)
        }
        
        access(all) fun clearAllPendingUpdates() {
            SpottedBassCoin.pendingUpdates = []
        }
        
        // ENHANCED ADMIN FUNCTIONS WITH VALIDATION
        access(all) fun updateRarityTierValidated(newTier: UInt8) {
            pre {
                SpottedBassCoin.validateRating(newTier): "Invalid rarity tier (must be 1-10)"
            }
            let oldTier = SpottedBassCoin.speciesMetadata.rarityTier?.toString() ?? "0"
            SpottedBassCoin.speciesMetadata.setRarityTier(newTier)
            emit MetadataUpdated(field: "rarityTier", oldValue: oldTier, newValue: newTier.toString())
        ...
        access(all) fun updateConservationStatusValidated(newStatus: String) {
            pre {
                SpottedBassCoin.validateConservationStatus(newStatus): "Invalid conservation status"
            }
            let oldStatus = SpottedBassCoin.speciesMetadata.globalConservationStatus ?? ""
            SpottedBassCoin.speciesMetadata.setConservationStatus(newStatus)
            emit MetadataUpdated(field: "globalConservationStatus", oldValue: oldStatus, newValue: newStatus)
        ...
        access(all) fun updateFightRatingValidated(newRating: UInt8) {
            pre {
                SpottedBassCoin.validateRating(newRating): "Fight rating must be 1-10"
            }
            let oldRating = SpottedBassCoin.speciesMetadata.fightRating?.toString() ?? "0"
            SpottedBassCoin.speciesMetadata.setFightRating(newRating)
        ...
        access(all) fun archiveCurrentYear() {
            let currentYear = SpottedBassCoin.currentMetadataYear
            SpottedBassCoin.metadataHistory[currentYear] = SpottedBassCoin.speciesMetadata
            emit YearlyMetadataCreated(year: currentYear)
        }
        
        access(all) fun updateToNewYear(_ newYear: UInt64) {
            pre {
                newYear > SpottedBassCoin.currentMetadataYear: "New year must be greater than current year"
            }
            // Archive current year data
            self.archiveCurrentYear()
            
            let oldYear = SpottedBassCoin.currentMetadataYear
            SpottedBassCoin.currentMetadataYear = newYear
            emit MetadataYearUpdated(oldYear: oldYear, newYear: newYear)
        }
    }

    // Public functions
    access(all) fun createEmptyVault(vaultType: Type): @SpottedBassCoin.Vault {
        pre {
            vaultType == Type<@SpottedBassCoin.Vault>(): "Vault type mismatch"
        }
        return <-create Vault(balance: 0.0)
    }
    ...
    // NOTE: SpottedBass coins represent permanent historical catch records
    ...
    access(all) view fun getVaultBalance(vaultRef: &SpottedBassCoin.Vault): UFix64 {
        return vaultRef.balance
    }
    
    access(all) view fun canWithdraw(vaultRef: &SpottedBassCoin.Vault, amount: UFix64): Bool {
    ...
    access(all) view fun isValidVaultType(vaultType: Type): Bool {
        return vaultType == Type<@SpottedBassCoin.Vault>()
    }
    ...
    init() {
        ...
        self.speciesMetadata = SpeciesMetadata(
            // IMMUTABLE CORE FIELDS
            speciesCode: "MICROPTERUS_PUNCTULATUS",
            ticker: "MICPUN",
            ...
            habitat: "Rocky streams, rivers, clear lakes, and reservoirs",
            averageWeight: 2.0,
            averageLength: 14.0,
            imageURL: "https://derby.fish/images/species/SpottedBass.jpg",
            description: "Spotted Bass are aggressive freshwater gamefish known for their distinctive spotted patterns and strong fighting ability. They prefer clear, rocky waters and are often called 'Kentucky Bass' due to their abundance in that region.",
            firstCatchDate: nil,
            rarityTier: 2,
            ...
            regionalPopulations: {
                "Southeast": RegionalPopulation(
                    populationTrend: "Stable",
                    threats: ["Habitat Modification", "Water Quality", "Climate Change"],
                    protectedAreas: ["National Forests", "State Parks", "Wildlife Management Areas"],
                    estimatedPopulation: nil
                ),
                "Ohio River Basin": RegionalPopulation(
                    populationTrend: "Stable",
                    threats: ["Pollution", "Dam Construction", "Habitat Loss"],
                    protectedAreas: ["Ohio River Islands National Wildlife Refuge"],
                    estimatedPopulation: nil
                ),
                "Texas Hill Country": RegionalPopulation(
                    populationTrend: "Stable",
                    threats: ["Water Level Fluctuations", "Development", "Overfishing"],
                    protectedAreas: ["Guadalupe River State Park", "Various State Parks"],
                    estimatedPopulation: nil
                )
            },
            ...
            // BIOLOGICAL INTELLIGENCE
            lifespan: 12.0,
            diet: "Crayfish, small fish, insects, worms, minnows",
            predators: ["Larger bass", "Northern Pike", "Muskie", "Birds", "Humans"],
            temperatureRange: "55-85°F (optimal 70-80°F)",
            depthRange: "3-40 feet",
            spawningAge: 2.0,
            spawningBehavior: "Builds gravel nests in shallow areas, male guards eggs and fry",
            migrationPattern: "Seasonal movements to deeper water in winter, shallow areas for spawning",
            waterQualityNeeds: "Clear water preferred, pH 6.5-8.5, moderate to high dissolved oxygen",
            
            // GEOGRAPHIC & HABITAT
            nativeRegions: ["Southeast US", "Ohio River Basin", "Gulf Coast Plains"],
            currentRange: ["Southeast US", "South-Central US", "Introduced to Western US"],
            waterTypes: ["Stream", "River", "Lake", "Reservoir", "Creek"],
            ...
            tourismValue: 7,
            ecosystemRole: "Mid-level Predator, Sport Fish",
            culturalSignificance: "Popular tournament fish, important to bass fishing culture",
            
            // ANGLING & RECREATIONAL
            bestBaits: ["Jigs", "Soft plastics", "Spinnerbaits", "Crankbaits", "Live minnows"],
            fightRating: 7,
            culinaryRating: 7,
            catchDifficulty: 5,
            seasonalAvailability: "Year-round, best spring and fall",
            bestTechniques: ["Casting", "Jigging", "Drop shot", "Carolina rig", "Finesse fishing"],
            
            // REGULATORY
            regionalRegulations: {
                "Alabama": RegionalRegulations(
                    sizeLimit: 12.0,
                    bagLimit: 6,
                    closedSeasons: [],
                    specialRegulations: "Combined bass limit in some waters",
                    licenseRequired: true
                ),
                "Kentucky": RegionalRegulations(
                    sizeLimit: 12.0,
                    bagLimit: 6,
                    closedSeasons: [],
                    specialRegulations: "Special regulations on some streams",
                    licenseRequired: true
                ),
                "Texas": RegionalRegulations(
                    ...
                    bagLimit: 5,
                    closedSeasons: [],
                    specialRegulations: "Only one bass over 16 inches",
                    licenseRequired: true
                )
            },
            
            // PHYSICAL & BEHAVIORAL
            physicalDescription: "Olive-green with dark lateral blotches, rows of dark spots below lateral line, tooth patch on tongue",
            behaviorTraits: "Aggressive predator, structure-oriented, schooling behavior in open water",
            seasonalPatterns: "Spring spawning in shallows, summer deep structure, fall feeding, winter dormancy",
            
            // RECORDS & ACHIEVEMENTS
            recordWeight: 11.15,
            recordWeightLocation: "Pine Flat Lake, California",
            recordWeightDate: "March 18, 2017",
            recordLength: 25.0,
            recordLengthLocation: "Dale Hollow Lake, Tennessee",
            recordLengthDate: "April 12, 2014",
            
            // RESEARCH & SCIENTIFIC
            researchPriority: 6,
            geneticMarkers: "Studies on hybridization with largemouth bass",
            studyPrograms: ["State Fish and Wildlife Agencies", "University Research Programs", "Bass Tournament Organizations"],
            ...
            additionalMetadata: {
                ...
                "also_known_as": "Kentucky Bass",
                "nickname": "Spots"
            }
        )

        // Set storage paths using common name format
        self.VaultStoragePath = /storage/SpottedBassCoinVault
        self.VaultPublicPath = /public/SpottedBassCoinReceiver
        self.MinterStoragePath = /storage/SpottedBassCoinMinter
        self.MetadataAdminStoragePath = /storage/SpottedBassCoinMetadataAdmin
        self.FishDEXCoordinatorStoragePath = /storage/SpottedBassCoinFishDEXCoordinator
        self.FishDEXCoordinatorPublicPath = /public/SpottedBassCoinFishDEXCoordinator
        ...
        let cap = self.account.capabilities.storage.issue<&SpottedBassCoin.Vault>(self.VaultStoragePath)
