oin.Vault>()
    }
    ...
    init() {
        ...
        self.speciesMetadata = SpeciesMetadata(
            // IMMUTABLE CORE FIELDS
            speciesCode: "CORYPHAENA_HIPPURUS",
            ticker: "CORHIP",
            ...
            averageWeight: 25.0,
            averageLength: 36.0,
            imageURL: "https://derby.fish/images/species/MahiMahi.jpg",
            description: "Mahi Mahi, also known as dolphinfish or dorado, are spectacular pelagic gamefish known for their brilliant colors and acrobatic fights. They are fast-growing, short-lived fish that are highly prized for both sport and table fare.",
            firstCatchDate: nil,
            rarityTier: 3,
            ...
            regionalPopulations: {
                "Atlantic Ocean": RegionalPopulation(
                    ...
                    protectedAreas: ["Atlantic Marine Protected Areas"],
                    estimatedPopulation: nil
                ),
                "Pacific Ocean": RegionalPopulation(
                    populationTrend: "Stable",
                    threats: ["Commercial Fishing", "Habitat Changes", "El Niño Effects"],
                    protectedAreas: ["Pacific Marine Sanctuaries"],
                    estimatedPopulation: nil
                ),
                "Gulf of Mexico": RegionalPopulation(
                    populationTrend: "Stable",
                    threats: ["Longline Fishing", "Pollution", "Climate Change"],
                    protectedAreas: ["Gulf Marine Protected Areas"],
                    estimatedPopulation: nil
                )
            },
            ...
            // BIOLOGICAL INTELLIGENCE
            lifespan: 5.0,
            diet: "Flying fish, squid, crabs, small fish, juvenile sea turtles",
            predators: ["Sharks", "Billfish", "Tuna", "Wahoo", "Humans"],
            temperatureRange: "70-88°F (optimal 75-85°F)",
            depthRange: "0-300 feet",
            spawningAge: 1.0,
            spawningBehavior: "Spawns year-round in warm waters, releases pelagic eggs",
            migrationPattern: "Follows warm currents and floating debris, highly migratory",
            waterQualityNeeds: "Clean tropical oceanic water with high dissolved oxygen",
            
            // GEOGRAPHIC & HABITAT
            nativeRegions: ["Tropical Atlantic", "Pacific Ocean", "Indian Ocean"],
            currentRange: ["Worldwide tropical and subtropical waters"],
            waterTypes: ["Open Ocean", "Pelagic", "Offshore Waters"],
            ...
            tourismValue: 9,
            ecosystemRole: "Mid-level Predator, Fast Converter",
            culturalSignificance: "Important commercial and recreational species, Hawaiian cultural significance",
            
            // ANGLING & RECREATIONAL
            bestBaits: ["Ballyhoo", "Squid", "Flying fish", "Artificial lures", "Live bait"],
            fightRating: 9,
            culinaryRating: 10,
            catchDifficulty: 6,
            seasonalAvailability: "Year-round in tropics, seasonal migrations",
            bestTechniques: ["Trolling", "Casting to debris", "Live bait fishing", "Fly fishing", "Kite fishing"],
            ...
            regionalRegulations: {
                "Florida": RegionalRegulations(
                    ...
                    bagLimit: 10,
                    closedSeasons: [],
                    specialRegulations: "Federal waters regulations apply",
                    licenseRequired: true
                ),
                "Hawaii": RegionalRegulations(
                    sizeLimit: 0.0,
                    bagLimit: 10,
                    closedSeasons: [],
                    specialRegulations: "No minimum size",
                    licenseRequired: true
                ),
                "Federal Waters": RegionalRegulations(
                    sizeLimit: 20.0,
                    bagLimit: 54,
                    closedSeasons: [],
                    specialRegulations: "Per vessel per day",
                    licenseRequired: true
                )
            },
            
            // PHYSICAL & BEHAVIORAL
            physicalDescription: "Brilliant gold and green colors, blunt forehead, long dorsal fin, deeply forked tail",
            behaviorTraits: "Fast swimmer, aggressive feeder, attracted to floating objects, schooling behavior",
            seasonalPatterns: "Follows warm water masses and current systems",
            
            // RECORDS & ACHIEVEMENTS
            recordWeight: 87.0,
            recordWeightLocation: "Papagayo Gulf, Costa Rica",
            recordWeightDate: "September 25, 1976",
            recordLength: 81.0,
            recordLengthLocation: "Pacific Ocean, Mexico",
            recordLengthDate: "June 12, 1998",
            
            // RESEARCH & SCIENTIFIC
            researchPriority: 7,
            geneticMarkers: "Population structure and migration studies",
            studyPrograms: ["NOAA Fisheries", "International Commission for Conservation of Atlantic Tunas", "Pacific Fishery Management Council"],
        ...
        self.VaultStoragePath = /storage/MahiMahiCoinVault
        self.VaultPublicPath = /public/MahiMahiCoinReceiver
        self.MinterStoragePath = /storage/MahiMahiCoinMinter
        self.MetadataAdminStoragePath = /storage/MahiMahiCoinMetadataAdmin
        self.FishDEXCoordinatorStoragePath = /storage/MahiMahiCoinFishDEXCoordinator
        self.FishDEXCoordinatorPublicPath = /public/MahiMahiCoinFishDEXCoordinator
        ...
        let cap = self.account.capabilities.storage.issue<&MahiMahiCoin.Vault>(self.VaultStoragePath)
