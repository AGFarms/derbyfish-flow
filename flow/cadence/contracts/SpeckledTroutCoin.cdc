...
access(all) contract SpeckledTroutCoin: FungibleToken {
    ...
    access(all) resource interface SpeciesCoinPublic {
        ...
        access(all) fun redeemCatchNFT(fishData: {String: AnyStruct}, angler: Address): @SpeckledTroutCoin.Vault
    }
        ...
        switch viewType {
            ...
            case Type<FungibleTokenMetadataViews.FTVaultData>():
                return FungibleTokenMetadataViews.FTVaultData(
                    ...
                    receiverLinkedType: Type<&SpeckledTroutCoin.Vault>(),
                    metadataLinkedType: Type<&SpeckledTroutCoin.Vault>(),
                    createEmptyVaultFunction: (fun(): @{FungibleToken.Vault} {
                        return <-SpeckledTroutCoin.createEmptyVault(vaultType: Type<@SpeckledTroutCoin.Vault>())
                    })
                )
    ...
    access(all) resource FishDEXCoordinator: SpeciesCoinPublic {
        ...
        access(all) view fun getSpeciesCode(): String {
            return SpeckledTroutCoin.speciesMetadata.speciesCode
        }
        
        access(all) view fun getTicker(): String {
            return SpeckledTroutCoin.speciesMetadata.ticker
        }
        
        access(all) view fun getCommonName(): String {
            return SpeckledTroutCoin.speciesMetadata.commonName
        }
        
        access(all) view fun getScientificName(): String {
            return SpeckledTroutCoin.speciesMetadata.scientificName
        }
        
        access(all) view fun getFamily(): String {
            return SpeckledTroutCoin.speciesMetadata.family
        }
        
        access(all) view fun getTotalSupply(): UFix64 {
            return SpeckledTroutCoin.totalSupply
        }
        ...
        access(all) view fun getBasicRegistryInfo(): {String: AnyStruct} {
            return {
                ...
                "contractAddress": SpeckledTroutCoin.account.address,
                "dataYear": SpeckledTroutCoin.speciesMetadata.dataYear,
                "conservationStatus": SpeckledTroutCoin.speciesMetadata.globalConservationStatus,
                "rarityTier": SpeckledTroutCoin.speciesMetadata.rarityTier,
                "isRegistered": SpeckledTroutCoin.isRegisteredWithFishDEX
            }
        }
        ...
        access(all) fun redeemCatchNFT(fishData: {String: AnyStruct}, angler: Address): @SpeckledTroutCoin.Vault {
            ...
            if let nftId = fishNFTId {
                assert(
                    SpeckledTroutCoin.fishNFTRegistry[nftId] == nil,
                    message: "This NFT has already been redeemed for a coin"
                )
                // Record this NFT as redeemed
                SpeckledTroutCoin.fishNFTRegistry[nftId] = true
            }
            
            // Auto-record first catch if this is the very first mint
            if SpeckledTroutCoin.totalSupply == 0.0 && SpeckledTroutCoin.speciesMetadata.firstCatchDate == nil {
                SpeckledTroutCoin.speciesMetadata.setFirstCatchDate(UInt64(getCurrentBlock().timestamp))
            ...
            SpeckledTroutCoin.totalSupply = SpeckledTroutCoin.totalSupply + amount
        ...
        access(all) fun registerWithFishDEX(fishDEXAddress: Address) {
            pre {
                !SpeckledTroutCoin.isRegisteredWithFishDEX: "Already registered with FishDEX"
            }
            ...
            SpeckledTroutCoin.fishDEXAddress = fishDEXAddress
            SpeckledTroutCoin.isRegisteredWithFishDEX = true
        ...
        access(all) fun updateFishDEXAddress(newAddress: Address) {
            let oldAddress = SpeckledTroutCoin.fishDEXAddress
            SpeckledTroutCoin.fishDEXAddress = newAddress
            SpeckledTroutCoin.isRegisteredWithFishDEX = false // Reset registration status
    ...
    access(all) resource Vault: FungibleToken.Vault {
        ...
        access(contract) fun burnCallback() {
            if self.balance > 0.0 {
                SpeckledTroutCoin.totalSupply = SpeckledTroutCoin.totalSupply - self.balance
        ...
        access(all) view fun getViews(): [Type] {
            return SpeckledTroutCoin.getContractViews(resourceType: nil)
        }

        access(all) fun resolveView(_ view: Type): AnyStruct? {
            return SpeckledTroutCoin.resolveContractView(resourceType: nil, viewType: view)
        }

        access(all) view fun getSupportedVaultTypes(): {Type: Bool} {
            return {Type<@SpeckledTroutCoin.Vault>(): true}
        }

        access(all) view fun isSupportedVaultType(type: Type): Bool {
            return type == Type<@SpeckledTroutCoin.Vault>()
        }
        ...
        access(FungibleToken.Withdraw) fun withdraw(amount: UFix64): @SpeckledTroutCoin.Vault {
            self.balance = self.balance - amount
            return <-create Vault(balance: amount)
        }

        access(all) fun deposit(from: @{FungibleToken.Vault}) {
            let vault <- from as! @SpeckledTroutCoin.Vault
            self.balance = self.balance + vault.balance
            vault.balance = 0.0
            destroy vault
        }

        access(all) fun createEmptyVault(): @SpeckledTroutCoin.Vault {
    ...
    access(all) resource Minter {
        ...
        access(all) fun mintForCatch(amount: UFix64, fishId: UInt64, angler: Address): @SpeckledTroutCoin.Vault {
            pre {
                amount == 1.0: "Only 1 coin per verified catch"
                SpeckledTroutCoin.fishNFTRegistry[fishId] == nil: "This NFT has already been used to mint a coin"
            }
            
            // Record this NFT as used
            SpeckledTroutCoin.fishNFTRegistry[fishId] = true
        ...
        access(all) fun autoRegisterWithFishDEX() {
            if let fishDEXAddr = SpeckledTroutCoin.fishDEXAddress {
                if !SpeckledTroutCoin.isRegisteredWithFishDEX {
                    // Get reference to FishDEX coordinator
                    if let coordinatorRef = SpeckledTroutCoin.account.capabilities.borrow<&FishDEXCoordinator>(
                        SpeckledTroutCoin.FishDEXCoordinatorPublicPath
                    ) {
        ...
        access(all) fun mintBatch(recipients: {Address: UFix64}): @{Address: SpeckledTroutCoin.Vault} {
            let vaults: @{Address: SpeckledTroutCoin.Vault} <- {}
    ...
    access(all) resource MetadataAdmin {
        ...
        access(all) fun updateImageURL(newURL: String) {
            let oldURL = SpeckledTroutCoin.speciesMetadata.imageURL ?? ""
            SpeckledTroutCoin.speciesMetadata.setImageURL(newURL)
        ...
        access(all) fun updateDescription(newDescription: String) {
            let oldDescription = SpeckledTroutCoin.speciesMetadata.description
            SpeckledTroutCoin.speciesMetadata.setDescription(newDescription)
        ...
        access(all) fun updateHabitat(newHabitat: String) {
            let oldHabitat = SpeckledTroutCoin.speciesMetadata.habitat ?? ""
            SpeckledTroutCoin.speciesMetadata.setHabitat(newHabitat)
        ...
        access(all) fun updateAverageWeight(newWeight: UFix64) {
            let oldWeight = SpeckledTroutCoin.speciesMetadata.averageWeight?.toString() ?? "0.0"
            SpeckledTroutCoin.speciesMetadata.setAverageWeight(newWeight)
        ...
        access(all) fun updateAverageLength(newLength: UFix64) {
            let oldLength = SpeckledTroutCoin.speciesMetadata.averageLength?.toString() ?? "0.0"
            SpeckledTroutCoin.speciesMetadata.setAverageLength(newLength)
        ...
        access(all) fun updateRarityTier(newTier: UInt8) {
            ...
            let oldTier = SpeckledTroutCoin.speciesMetadata.rarityTier?.toString() ?? "0"
            SpeckledTroutCoin.speciesMetadata.setRarityTier(newTier)
        ...
        access(all) fun manuallySetFirstCatch(timestamp: UInt64, angler: Address) {
            pre {
                SpeckledTroutCoin.speciesMetadata.firstCatchDate == nil: "First catch already recorded"
            }
            SpeckledTroutCoin.speciesMetadata.setFirstCatchDate(timestamp)
        ...
        access(all) fun updateConservationStatus(newStatus: String) {
            let oldStatus = SpeckledTroutCoin.speciesMetadata.globalConservationStatus ?? ""
            SpeckledTroutCoin.speciesMetadata.setConservationStatus(newStatus)
        ...
        access(all) fun updateNativeRegions(newRegions: [String]) {
            var oldRegionsStr = "["
            for i, region in SpeckledTroutCoin.speciesMetadata.nativeRegions {
                if i > 0 { oldRegionsStr = oldRegionsStr.concat(",") }
                oldRegionsStr = oldRegionsStr.concat(region)
            }
            oldRegionsStr = oldRegionsStr.concat("]")
            
            SpeckledTroutCoin.speciesMetadata.setNativeRegions(newRegions)
        ...
        access(all) fun updateSeasonalPatterns(newPatterns: String) {
            let oldPatterns = SpeckledTroutCoin.speciesMetadata.seasonalPatterns ?? ""
            SpeckledTroutCoin.speciesMetadata.setSeasonalPatterns(newPatterns)
        ...
        access(all) fun updateRecordWeight(newRecord: UFix64) {
            let oldRecord = SpeckledTroutCoin.speciesMetadata.recordWeight?.toString() ?? "0.0"
            SpeckledTroutCoin.speciesMetadata.setRecordWeight(newRecord)
        ...
        access(all) fun updateRecordLength(newRecord: UFix64) {
            let oldRecord = SpeckledTroutCoin.speciesMetadata.recordLength?.toString() ?? "0.0"
            SpeckledTroutCoin.speciesMetadata.setRecordLength(newRecord)
        ...
        access(all) fun updateRecordWeightLocation(newLocation: String?) {
            let oldLocation = SpeckledTroutCoin.speciesMetadata.recordWeightLocation ?? ""
            SpeckledTroutCoin.speciesMetadata.setRecordWeightLocation(newLocation)
        ...
        access(all) fun updateRecordWeightDate(newDate: String?) {
            let oldDate = SpeckledTroutCoin.speciesMetadata.recordWeightDate ?? ""
            SpeckledTroutCoin.speciesMetadata.setRecordWeightDate(newDate)
        ...
        access(all) fun updateRecordLengthLocation(newLocation: String?) {
            let oldLocation = SpeckledTroutCoin.speciesMetadata.recordLengthLocation ?? ""
            SpeckledTroutCoin.speciesMetadata.setRecordLengthLocation(newLocation)
        ...
        access(all) fun updateRecordLengthDate(newDate: String?) {
            let oldDate = SpeckledTroutCoin.speciesMetadata.recordLengthDate ?? ""
            SpeckledTroutCoin.speciesMetadata.setRecordLengthDate(newDate)
        ...
        access(all) fun approvePendingUpdate(index: Int) {
            pre {
                index >= 0 && index < SpeckledTroutCoin.pendingUpdates.length: "Invalid update index"
            }
            let update = SpeckledTroutCoin.pendingUpdates.remove(at: index)
        ...
        access(all) fun rejectPendingUpdate(index: Int) {
            ...
            SpeckledTroutCoin.pendingUpdates.remove(at: index)
        }
        
        access(all) fun clearAllPendingUpdates() {
            SpeckledTroutCoin.pendingUpdates = []
        }
        
        // ENHANCED ADMIN FUNCTIONS WITH VALIDATION
        access(all) fun updateRarityTierValidated(newTier: UInt8) {
            pre {
                SpeckledTroutCoin.validateRating(newTier): "Invalid rarity tier (must be 1-10)"
            }
            let oldTier = SpeckledTroutCoin.speciesMetadata.rarityTier?.toString() ?? "0"
            SpeckledTroutCoin.speciesMetadata.setRarityTier(newTier)
            emit MetadataUpdated(field: "rarityTier", oldValue: oldTier, newValue: newTier.toString())
        ...
        access(all) fun updateConservationStatusValidated(newStatus: String) {
            pre {
                SpeckledTroutCoin.validateConservationStatus(newStatus): "Invalid conservation status"
            }
            let oldStatus = SpeckledTroutCoin.speciesMetadata.globalConservationStatus ?? ""
            SpeckledTroutCoin.speciesMetadata.setConservationStatus(newStatus)
            emit MetadataUpdated(field: "globalConservationStatus", oldValue: oldStatus, newValue: newStatus)
        ...
        access(all) fun updateFightRatingValidated(newRating: UInt8) {
            pre {
                SpeckledTroutCoin.validateRating(newRating): "Fight rating must be 1-10"
            }
            let oldRating = SpeckledTroutCoin.speciesMetadata.fightRating?.toString() ?? "0"
            SpeckledTroutCoin.speciesMetadata.setFightRating(newRating)
        ...
        access(all) fun archiveCurrentYear() {
            let currentYear = SpeckledTroutCoin.currentMetadataYear
            SpeckledTroutCoin.metadataHistory[currentYear] = SpeckledTroutCoin.speciesMetadata
            emit YearlyMetadataCreated(year: currentYear)
        }
        
        access(all) fun updateToNewYear(_ newYear: UInt64) {
            pre {
                newYear > SpeckledTroutCoin.currentMetadataYear: "New year must be greater than current year"
            }
            // Archive current year data
            self.archiveCurrentYear()
            
            let oldYear = SpeckledTroutCoin.currentMetadataYear
            SpeckledTroutCoin.currentMetadataYear = newYear
            emit MetadataYearUpdated(oldYear: oldYear, newYear: newYear)
        }
    }

    // Public functions
    access(all) fun createEmptyVault(vaultType: Type): @SpeckledTroutCoin.Vault {
        pre {
            vaultType == Type<@SpeckledTroutCoin.Vault>(): "Vault type mismatch"
        }
        return <-create Vault(balance: 0.0)
    }
    ...
    // NOTE: SpeckledTrout coins represent permanent historical catch records
    ...
    access(all) view fun getVaultBalance(vaultRef: &SpeckledTroutCoin.Vault): UFix64 {
        return vaultRef.balance
    }
    
    access(all) view fun canWithdraw(vaultRef: &SpeckledTroutCoin.Vault, amount: UFix64): Bool {
    ...
    access(all) view fun isValidVaultType(vaultType: Type): Bool {
        return vaultType == Type<@SpeckledTroutCoin.Vault>()
    }
    ...
    init() {
        ...
        self.speciesMetadata = SpeciesMetadata(
            // IMMUTABLE CORE FIELDS
            speciesCode: "CYNOSCION_NEBULOSUS",
            ticker: "CYNNEB",
            ...
            habitat: "Coastal waters, estuaries, grass flats, and shallow bays",
            averageWeight: 3.5,
            averageLength: 18.0,
            imageURL: "https://derby.fish/images/species/SpeckledTrout.jpg",
            description: "Spotted Seatrout, also known as Speckled Trout, are popular inshore gamefish known for their distinctive spotted patterns and excellent eating quality. They are not true trout but members of the drum family, preferring shallow coastal waters.",
            firstCatchDate: nil,
            rarityTier: 2,
            ...
            regionalPopulations: {
                "Gulf of Mexico": RegionalPopulation(
                    ...
                    protectedAreas: ["Gulf Coast Marine Protected Areas", "Estuarine Reserves"],
                    estimatedPopulation: nil
                ),
                "Atlantic Coast": RegionalPopulation(
                    populationTrend: "Stable",
                    threats: ["Coastal Development", "Pollution", "Climate Change"],
                    protectedAreas: ["Atlantic Coast Marine Sanctuaries"],
                    estimatedPopulation: nil
                ),
                "Texas Coast": RegionalPopulation(
                    populationTrend: "Stable",
                    threats: ["Red Tide", "Freshwater Inflows", "Habitat Modification"],
                    protectedAreas: ["Texas State Parks", "Marine Protected Areas"],
                    estimatedPopulation: nil
                )
            },
            ...
            // BIOLOGICAL INTELLIGENCE
            lifespan: 10.0,
            diet: "Shrimp, crabs, small fish, marine worms, squid",
            predators: ["Sharks", "Larger fish", "Birds", "Dolphins", "Humans"],
            temperatureRange: "65-85°F (optimal 72-80°F)",
            depthRange: "0-30 feet",
            spawningAge: 2.0,
            spawningBehavior: "Spawns in coastal waters, releases pelagic eggs in spring and summer",
            migrationPattern: "Moves to deeper water in winter, shallow areas in spring and summer",
            waterQualityNeeds: "Clean saltwater, moderate salinity, good dissolved oxygen",
            
            // GEOGRAPHIC & HABITAT
            nativeRegions: ["Gulf of Mexico", "Atlantic Coast", "Caribbean"],
            currentRange: ["Gulf Coast", "Atlantic Coast from Florida to Virginia"],
            waterTypes: ["Estuarine", "Coastal", "Grass Flats", "Shallow Bays"],
            ...
            culturalSignificance: "State saltwater fish of Texas, important to Gulf Coast fishing culture",
            
            // ANGLING & RECREATIONAL
            bestBaits: ["Live shrimp", "Soft plastics", "Topwater lures", "Spoons", "Jigs"],
            fightRating: 6,
            culinaryRating: 9,
            catchDifficulty: 4,
            seasonalAvailability: "Year-round in south, spring through fall in north",
            bestTechniques: ["Wade fishing", "Drift fishing", "Sight casting", "Popping cork", "Artificial lures"],
            ...
            regionalRegulations: {
                "Texas": RegionalRegulations(
                    ...
                    bagLimit: 10,
                    closedSeasons: [],
                    specialRegulations: "Only one fish over 25 inches",
                ...
                "Louisiana": RegionalRegulations(
                    ...
                    bagLimit: 25,
                    closedSeasons: [],
                    specialRegulations: "Recreational license required",
                    licenseRequired: true
                ),
                "Florida": RegionalRegulations(
                    ...
                    bagLimit: 4,
                    closedSeasons: [],
                    specialRegulations: "Slot limit 15-20 inches, one over 20 allowed",
                    licenseRequired: true
                )
            },
            
            // PHYSICAL & BEHAVIORAL
            physicalDescription: "Silver body with distinctive black spots on sides and fins, elongated soft dorsal fin, protruding canine teeth",
            behaviorTraits: "Schooling behavior, structure-oriented, feeds actively at dawn and dusk",
            seasonalPatterns: "Spring spawning runs, summer shallow feeding, winter deeper water retreat",
            
            // RECORDS & ACHIEVEMENTS
            recordWeight: 17.7,
            recordWeightLocation: "Fort Pierce, Florida",
            recordWeightDate: "May 11, 1995",
            recordLength: 35.0,
            recordLengthLocation: "Indian River, Florida",
            recordLengthDate: "March 15, 2000",
            
            // RESEARCH & SCIENTIFIC
            researchPriority: 7,
            geneticMarkers: "Studies on stock structure and habitat requirements",
            studyPrograms: ["Gulf States Marine Fisheries Commission", "NOAA Fisheries", "State Marine Resource Agencies"],
            ...
            additionalMetadata: {
                ...
                "state_fish": "Texas (saltwater)",
                "nickname": "Specks"
            }
        )

        // Set storage paths using common name format
        self.VaultStoragePath = /storage/SpeckledTroutCoinVault
        self.VaultPublicPath = /public/SpeckledTroutCoinReceiver
        self.MinterStoragePath = /storage/SpeckledTroutCoinMinter
        self.MetadataAdminStoragePath = /storage/SpeckledTroutCoinMetadataAdmin
        self.FishDEXCoordinatorStoragePath = /storage/SpeckledTroutCoinFishDEXCoordinator
        self.FishDEXCoordinatorPublicPath = /public/SpeckledTroutCoinFishDEXCoordinator
        ...
        let cap = self.account.capabilities.storage.issue<&SpeckledTroutCoin.Vault>(self.VaultStoragePath)
