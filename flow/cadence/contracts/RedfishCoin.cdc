...
access(all) contract RedfishCoin: FungibleToken {
    ...
    access(all) resource interface SpeciesCoinPublic {
        ...
        access(all) fun redeemCatchNFT(fishData: {String: AnyStruct}, angler: Address): @RedfishCoin.Vault
    }
        ...
        switch viewType {
            ...
            case Type<FungibleTokenMetadataViews.FTVaultData>():
                return FungibleTokenMetadataViews.FTVaultData(
                    ...
                    receiverLinkedType: Type<&RedfishCoin.Vault>(),
                    metadataLinkedType: Type<&RedfishCoin.Vault>(),
                    createEmptyVaultFunction: (fun(): @{FungibleToken.Vault} {
                        return <-RedfishCoin.createEmptyVault(vaultType: Type<@RedfishCoin.Vault>())
                    })
                )
    ...
    access(all) resource FishDEXCoordinator: SpeciesCoinPublic {
        ...
        access(all) view fun getSpeciesCode(): String {
            return RedfishCoin.speciesMetadata.speciesCode
        }
        
        access(all) view fun getTicker(): String {
            return RedfishCoin.speciesMetadata.ticker
        }
        
        access(all) view fun getCommonName(): String {
            return RedfishCoin.speciesMetadata.commonName
        }
        
        access(all) view fun getScientificName(): String {
            return RedfishCoin.speciesMetadata.scientificName
        }
        
        access(all) view fun getFamily(): String {
            return RedfishCoin.speciesMetadata.family
        }
        
        access(all) view fun getTotalSupply(): UFix64 {
            return RedfishCoin.totalSupply
        }
        ...
        access(all) view fun getBasicRegistryInfo(): {String: AnyStruct} {
            return {
                ...
                "contractAddress": RedfishCoin.account.address,
                "dataYear": RedfishCoin.speciesMetadata.dataYear,
                "conservationStatus": RedfishCoin.speciesMetadata.globalConservationStatus,
                "rarityTier": RedfishCoin.speciesMetadata.rarityTier,
                "isRegistered": RedfishCoin.isRegisteredWithFishDEX
            }
        }
        ...
        access(all) fun redeemCatchNFT(fishData: {String: AnyStruct}, angler: Address): @RedfishCoin.Vault {
            ...
            if let nftId = fishNFTId {
                assert(
                    RedfishCoin.fishNFTRegistry[nftId] == nil,
                    message: "This NFT has already been redeemed for a coin"
                )
                // Record this NFT as redeemed
                RedfishCoin.fishNFTRegistry[nftId] = true
            }
            
            // Auto-record first catch if this is the very first mint
            if RedfishCoin.totalSupply == 0.0 && RedfishCoin.speciesMetadata.firstCatchDate == nil {
                RedfishCoin.speciesMetadata.setFirstCatchDate(UInt64(getCurrentBlock().timestamp))
            ...
            RedfishCoin.totalSupply = RedfishCoin.totalSupply + amount
        ...
        access(all) fun registerWithFishDEX(fishDEXAddress: Address) {
            pre {
                !RedfishCoin.isRegisteredWithFishDEX: "Already registered with FishDEX"
            }
            ...
            RedfishCoin.fishDEXAddress = fishDEXAddress
            RedfishCoin.isRegisteredWithFishDEX = true
        ...
        access(all) fun updateFishDEXAddress(newAddress: Address) {
            let oldAddress = RedfishCoin.fishDEXAddress
            RedfishCoin.fishDEXAddress = newAddress
            RedfishCoin.isRegisteredWithFishDEX = false // Reset registration status
    ...
    access(all) resource Vault: FungibleToken.Vault {
        ...
        access(contract) fun burnCallback() {
            if self.balance > 0.0 {
                RedfishCoin.totalSupply = RedfishCoin.totalSupply - self.balance
        ...
        access(all) view fun getViews(): [Type] {
            return RedfishCoin.getContractViews(resourceType: nil)
        }

        access(all) fun resolveView(_ view: Type): AnyStruct? {
            return RedfishCoin.resolveContractView(resourceType: nil, viewType: view)
        }

        access(all) view fun getSupportedVaultTypes(): {Type: Bool} {
            return {Type<@RedfishCoin.Vault>(): true}
        }

        access(all) view fun isSupportedVaultType(type: Type): Bool {
            return type == Type<@RedfishCoin.Vault>()
        }
        ...
        access(FungibleToken.Withdraw) fun withdraw(amount: UFix64): @RedfishCoin.Vault {
            self.balance = self.balance - amount
            return <-create Vault(balance: amount)
        }

        access(all) fun deposit(from: @{FungibleToken.Vault}) {
            let vault <- from as! @RedfishCoin.Vault
            self.balance = self.balance + vault.balance
            vault.balance = 0.0
            destroy vault
        }

        access(all) fun createEmptyVault(): @RedfishCoin.Vault {
    ...
    access(all) resource Minter {
        ...
        access(all) fun mintForCatch(amount: UFix64, fishId: UInt64, angler: Address): @RedfishCoin.Vault {
            pre {
                amount == 1.0: "Only 1 coin per verified catch"
                RedfishCoin.fishNFTRegistry[fishId] == nil: "This NFT has already been used to mint a coin"
            }
            
            // Record this NFT as used
            RedfishCoin.fishNFTRegistry[fishId] = true
        ...
        access(all) fun autoRegisterWithFishDEX() {
            if let fishDEXAddr = RedfishCoin.fishDEXAddress {
                if !RedfishCoin.isRegisteredWithFishDEX {
                    // Get reference to FishDEX coordinator
                    if let coordinatorRef = RedfishCoin.account.capabilities.borrow<&FishDEXCoordinator>(
                        RedfishCoin.FishDEXCoordinatorPublicPath
                    ) {
        ...
        access(all) fun mintBatch(recipients: {Address: UFix64}): @{Address: RedfishCoin.Vault} {
            let vaults: @{Address: RedfishCoin.Vault} <- {}
    ...
    access(all) resource MetadataAdmin {
        ...
        access(all) fun updateImageURL(newURL: String) {
            let oldURL = RedfishCoin.speciesMetadata.imageURL ?? ""
            RedfishCoin.speciesMetadata.setImageURL(newURL)
        ...
        access(all) fun updateDescription(newDescription: String) {
            let oldDescription = RedfishCoin.speciesMetadata.description
            RedfishCoin.speciesMetadata.setDescription(newDescription)
        ...
        access(all) fun updateHabitat(newHabitat: String) {
            let oldHabitat = RedfishCoin.speciesMetadata.habitat ?? ""
            RedfishCoin.speciesMetadata.setHabitat(newHabitat)
        ...
        access(all) fun updateAverageWeight(newWeight: UFix64) {
            let oldWeight = RedfishCoin.speciesMetadata.averageWeight?.toString() ?? "0.0"
            RedfishCoin.speciesMetadata.setAverageWeight(newWeight)
        ...
        access(all) fun updateAverageLength(newLength: UFix64) {
            let oldLength = RedfishCoin.speciesMetadata.averageLength?.toString() ?? "0.0"
            RedfishCoin.speciesMetadata.setAverageLength(newLength)
        ...
        access(all) fun updateRarityTier(newTier: UInt8) {
            ...
            let oldTier = RedfishCoin.speciesMetadata.rarityTier?.toString() ?? "0"
            RedfishCoin.speciesMetadata.setRarityTier(newTier)
        ...
        access(all) fun manuallySetFirstCatch(timestamp: UInt64, angler: Address) {
            pre {
                RedfishCoin.speciesMetadata.firstCatchDate == nil: "First catch already recorded"
            }
            RedfishCoin.speciesMetadata.setFirstCatchDate(timestamp)
        ...
        access(all) fun updateConservationStatus(newStatus: String) {
            let oldStatus = RedfishCoin.speciesMetadata.globalConservationStatus ?? ""
            RedfishCoin.speciesMetadata.setConservationStatus(newStatus)
        ...
        access(all) fun updateSeasonalPatterns(newPatterns: String) {
            let oldPatterns = RedfishCoin.speciesMetadata.seasonalPatterns ?? ""
            RedfishCoin.speciesMetadata.setSeasonalPatterns(newPatterns)
        ...
        access(all) fun updateRecordWeight(newRecord: UFix64) {
            let oldRecord = RedfishCoin.speciesMetadata.recordWeight?.toString() ?? "0.0"
            RedfishCoin.speciesMetadata.setRecordWeight(newRecord)
        ...
        access(all) fun updateRecordLength(newRecord: UFix64) {
            let oldRecord = RedfishCoin.speciesMetadata.recordLength?.toString() ?? "0.0"
            RedfishCoin.speciesMetadata.setRecordLength(newRecord)
        ...
        access(all) fun updateRecordWeightLocation(newLocation: String?) {
            let oldLocation = RedfishCoin.speciesMetadata.recordWeightLocation ?? ""
            RedfishCoin.speciesMetadata.setRecordWeightLocation(newLocation)
        ...
        access(all) fun updateRecordWeightDate(newDate: String?) {
            let oldDate = RedfishCoin.speciesMetadata.recordWeightDate ?? ""
            RedfishCoin.speciesMetadata.setRecordWeightDate(newDate)
        ...
        access(all) fun updateRecordLengthLocation(newLocation: String?) {
            let oldLocation = RedfishCoin.speciesMetadata.recordLengthLocation ?? ""
            RedfishCoin.speciesMetadata.setRecordLengthLocation(newLocation)
        ...
        access(all) fun updateRecordLengthDate(newDate: String?) {
            let oldDate = RedfishCoin.speciesMetadata.recordLengthDate ?? ""
            RedfishCoin.speciesMetadata.setRecordLengthDate(newDate)
        ...
        access(all) fun approvePendingUpdate(index: Int) {
            pre {
                index >= 0 && index < RedfishCoin.pendingUpdates.length: "Invalid update index"
            }
            let update = RedfishCoin.pendingUpdates.remove(at: index)
        ...
        access(all) fun rejectPendingUpdate(index: Int) {
            ...
            RedfishCoin.pendingUpdates.remove(at: index)
        }
        
        access(all) fun clearAllPendingUpdates() {
            RedfishCoin.pendingUpdates = []
        }
        
        // ENHANCED ADMIN FUNCTIONS WITH VALIDATION
        access(all) fun updateRarityTierValidated(newTier: UInt8) {
            pre {
                RedfishCoin.validateRating(newTier): "Invalid rarity tier (must be 1-10)"
            }
            let oldTier = RedfishCoin.speciesMetadata.rarityTier?.toString() ?? "0"
            RedfishCoin.speciesMetadata.setRarityTier(newTier)
            emit MetadataUpdated(field: "rarityTier", oldValue: oldTier, newValue: newTier.toString())
        ...
        access(all) fun updateConservationStatusValidated(newStatus: String) {
            pre {
                RedfishCoin.validateConservationStatus(newStatus): "Invalid conservation status"
            }
            let oldStatus = RedfishCoin.speciesMetadata.globalConservationStatus ?? ""
            RedfishCoin.speciesMetadata.setConservationStatus(newStatus)
            emit MetadataUpdated(field: "globalConservationStatus", oldValue: oldStatus, newValue: newStatus)
        ...
        access(all) fun updateFightRatingValidated(newRating: UInt8) {
            pre {
                RedfishCoin.validateRating(newRating): "Fight rating must be 1-10"
            }
            let oldRating = RedfishCoin.speciesMetadata.fightRating?.toString() ?? "0"
            RedfishCoin.speciesMetadata.setFightRating(newRating)
        ...
        access(all) fun archiveCurrentYear() {
            let currentYear = RedfishCoin.currentMetadataYear
            RedfishCoin.metadataHistory[currentYear] = RedfishCoin.speciesMetadata
            emit YearlyMetadataCreated(year: currentYear)
        }
        
        access(all) fun updateToNewYear(_ newYear: UInt64) {
            pre {
                newYear > RedfishCoin.currentMetadataYear: "New year must be greater than current year"
            }
            // Archive current year data
            self.archiveCurrentYear()
            
            let oldYear = RedfishCoin.currentMetadataYear
            RedfishCoin.currentMetadataYear = newYear
            emit MetadataYearUpdated(oldYear: oldYear, newYear: newYear)
        }
    }

    // Public functions
    access(all) fun createEmptyVault(vaultType: Type): @RedfishCoin.Vault {
        pre {
            vaultType == Type<@RedfishCoin.Vault>(): "Vault type mismatch"
        }
        return <-create Vault(balance: 0.0)
    }
    ...
    // NOTE: Redfish coins represent permanent historical catch records
    ...
    access(all) view fun getVaultBalance(vaultRef: &RedfishCoin.Vault): UFix64 {
        return vaultRef.balance
    }
    
    access(all) view fun canWithdraw(vaultRef: &RedfishCoin.Vault, amount: UFix64): Bool {
    ...
    access(all) view fun isValidVaultType(vaultType: Type): Bool {
        return vaultType == Type<@RedfishCoin.Vault>()
    }
    ...
    init() {
        ...
        self.speciesMetadata = SpeciesMetadata(
            // IMMUTABLE CORE FIELDS
            speciesCode: "SCIAENOPS_OCELLATUS",
            ticker: "SCIOCE",
            ...
            // BASIC DESCRIPTIVE FIELDS
            commonName: "Red Drum",
            habitat: "Coastal waters, estuaries, surf zones, shallow flats",
            averageWeight: 15.0,
            averageLength: 30.0,
            imageURL: "https://derby.fish/images/species/Redfish.jpg",
            description: "Red Drum, commonly called Redfish, are iconic inshore gamefish known for their distinctive black spots and powerful fighting ability. They are highly sought after by anglers and are considered the premier sight-fishing species in shallow coastal waters.",
            firstCatchDate: nil,
            rarityTier: 3,
            ...
            regionalPopulations: {
                "Gulf of Mexico": RegionalPopulation(
                    ...
                    protectedAreas: ["Gulf Coast Marine Protected Areas", "Coastal Preserves"],
                    estimatedPopulation: nil
                ),
                "Atlantic Coast": RegionalPopulation(
                    populationTrend: "Stable",
                    threats: ["Coastal Development", "Pollution", "Hurricane Damage"],
                    protectedAreas: ["Atlantic Coast Marine Sanctuaries", "Estuarine Reserves"],
                    estimatedPopulation: nil
                ),
                "Texas Coast": RegionalPopulation(
                    populationTrend: "Stable",
                    threats: ["Freshwater Inflows", "Red Tide", "Habitat Modification"],
                    protectedAreas: ["Texas State Parks", "Coastal Preserves"],
                    estimatedPopulation: nil
                )
            },
            ...
            // BIOLOGICAL INTELLIGENCE
            lifespan: 50.0,
            diet: "Crabs, shrimp, small fish, mullet, menhaden, marine worms",
            predators: ["Sharks", "Larger fish", "Birds", "Humans"],
            temperatureRange: "65-85°F (optimal 70-80°F)",
            depthRange: "0-40 feet",
            spawningAge: 4.0,
            spawningBehavior: "Spawns near inlets and passes, releases pelagic eggs",
            migrationPattern: "Moves to deeper water in winter, shallow areas in spring and summer",
            waterQualityNeeds: "Saltwater, moderate salinity tolerance, good dissolved oxygen",
            
            // GEOGRAPHIC & HABITAT
            nativeRegions: ["Gulf of Mexico", "Atlantic Coast", "South Atlantic"],
            currentRange: ["Texas to North Carolina", "Gulf Coast", "Atlantic Coast"],
            waterTypes: ["Estuarine", "Coastal", "Surf Zone", "Shallow Flats"],
            ...
            tourismValue: 9,
            ecosystemRole: "Apex Predator (inshore), Keystone Species",
            culturalSignificance: "State fish of North Carolina, iconic Gulf Coast species",
            
            // ANGLING & RECREATIONAL
            bestBaits: ["Live shrimp", "Crabs", "Cut bait", "Spoons", "Soft plastics"],
            fightRating: 8,
            culinaryRating: 8,
            catchDifficulty: 5,
            seasonalAvailability: "Year-round in south, spring through fall in north",
            bestTechniques: ["Sight fishing", "Wade fishing", "Drift fishing", "Casting spoons", "Popping cork"],
            ...
            regionalRegulations: {
                "Texas": RegionalRegulations(
                    ...
                    bagLimit: 3,
                    closedSeasons: [],
                    specialRegulations: "One fish over 28 inches allowed",
                ...
                "Florida": RegionalRegulations(
                    ...
                    bagLimit: 1,
                    closedSeasons: [],
                    specialRegulations: "Slot limit 18-27 inches",
                    licenseRequired: true
                )
            },
            
            // PHYSICAL & BEHAVIORAL
            physicalDescription: "Copper-red body with distinctive black spots near tail, large scales, blunt snout",
            behaviorTraits: "Schooling behavior, shallow water feeder, strong fighter, temperature sensitive",
            seasonalPatterns: "Spring spawning runs, summer shallow feeding, winter deeper water movement",
            
            // RECORDS & ACHIEVEMENTS
            recordWeight: 94.2,
            recordWeightLocation: "Avon, North Carolina",
            recordWeightDate: "November 7, 1984",
            recordLength: 60.0,
            recordLengthLocation: "Cape Hatteras, North Carolina",
            recordLengthDate: "October 15, 1995",
            
            // RESEARCH & SCIENTIFIC
            researchPriority: 8,
            geneticMarkers: "Stock structure and habitat use studies",
            studyPrograms: ["Gulf States Marine Fisheries Commission", "Atlantic States Marine Fisheries Commission", "NOAA Fisheries"],
            ...
            additionalMetadata: {
                ...
                "state_fish": "North Carolina",
                "nickname": "Reds"
            }
        )

        // Set storage paths using common name format
        self.VaultStoragePath = /storage/RedfishCoinVault
        self.VaultPublicPath = /public/RedfishCoinReceiver
        self.MinterStoragePath = /storage/RedfishCoinMinter
        self.MetadataAdminStoragePath = /storage/RedfishCoinMetadataAdmin
        self.FishDEXCoordinatorStoragePath = /storage/RedfishCoinFishDEXCoordinator
        self.FishDEXCoordinatorPublicPath = /public/RedfishCoinFishDEXCoordinator
        ...
        let cap = self.account.capabilities.storage.issue<&RedfishCoin.Vault>(self.VaultStoragePath)
