...
access(all) contract BlueCatfishCoin: FungibleToken {
    ...
    access(all) resource interface SpeciesCoinPublic {
        ...
        access(all) fun redeemCatchNFT(fishData: {String: AnyStruct}, angler: Address): @BlueCatfishCoin.Vault
    }
        ...
        switch viewType {
            ...
            case Type<FungibleTokenMetadataViews.FTVaultData>():
                return FungibleTokenMetadataViews.FTVaultData(
                    ...
                    receiverLinkedType: Type<&BlueCatfishCoin.Vault>(),
                    metadataLinkedType: Type<&BlueCatfishCoin.Vault>(),
                    createEmptyVaultFunction: (fun(): @{FungibleToken.Vault} {
                        return <-BlueCatfishCoin.createEmptyVault(vaultType: Type<@BlueCatfishCoin.Vault>())
                    })
                )
    ...
    access(all) resource FishDEXCoordinator: SpeciesCoinPublic {
        ...
        access(all) view fun getSpeciesCode(): String {
            return BlueCatfishCoin.speciesMetadata.speciesCode
        }
        
        access(all) view fun getTicker(): String {
            return BlueCatfishCoin.speciesMetadata.ticker
        }
        
        access(all) view fun getCommonName(): String {
            return BlueCatfishCoin.speciesMetadata.commonName
        }
        
        access(all) view fun getScientificName(): String {
            return BlueCatfishCoin.speciesMetadata.scientificName
        }
        
        access(all) view fun getFamily(): String {
            return BlueCatfishCoin.speciesMetadata.family
        }
        
        access(all) view fun getTotalSupply(): UFix64 {
            return BlueCatfishCoin.totalSupply
        }
        ...
        access(all) view fun getBasicRegistryInfo(): {String: AnyStruct} {
            return {
                ...
                "contractAddress": BlueCatfishCoin.account.address,
                "dataYear": BlueCatfishCoin.speciesMetadata.dataYear,
                "conservationStatus": BlueCatfishCoin.speciesMetadata.globalConservationStatus,
                "rarityTier": BlueCatfishCoin.speciesMetadata.rarityTier,
                "isRegistered": BlueCatfishCoin.isRegisteredWithFishDEX
            }
        }
        ...
        access(all) fun redeemCatchNFT(fishData: {String: AnyStruct}, angler: Address): @BlueCatfishCoin.Vault {
            ...
            if let nftId = fishNFTId {
                assert(
                    BlueCatfishCoin.fishNFTRegistry[nftId] == nil,
                    message: "This NFT has already been redeemed for a coin"
                )
                // Record this NFT as redeemed
                BlueCatfishCoin.fishNFTRegistry[nftId] = true
            }
            
            // Auto-record first catch if this is the very first mint
            if BlueCatfishCoin.totalSupply == 0.0 && BlueCatfishCoin.speciesMetadata.firstCatchDate == nil {
                BlueCatfishCoin.speciesMetadata.setFirstCatchDate(UInt64(getCurrentBlock().timestamp))
            ...
            BlueCatfishCoin.totalSupply = BlueCatfishCoin.totalSupply + amount
        ...
        access(all) fun registerWithFishDEX(fishDEXAddress: Address) {
            pre {
                !BlueCatfishCoin.isRegisteredWithFishDEX: "Already registered with FishDEX"
            }
            ...
            BlueCatfishCoin.fishDEXAddress = fishDEXAddress
            BlueCatfishCoin.isRegisteredWithFishDEX = true
        ...
        access(all) fun updateFishDEXAddress(newAddress: Address) {
            let oldAddress = BlueCatfishCoin.fishDEXAddress
            BlueCatfishCoin.fishDEXAddress = newAddress
            BlueCatfishCoin.isRegisteredWithFishDEX = false // Reset registration status
    ...
    access(all) resource Vault: FungibleToken.Vault {
        ...
        access(contract) fun burnCallback() {
            if self.balance > 0.0 {
                BlueCatfishCoin.totalSupply = BlueCatfishCoin.totalSupply - self.balance
        ...
        access(all) view fun getViews(): [Type] {
            return BlueCatfishCoin.getContractViews(resourceType: nil)
        }

        access(all) fun resolveView(_ view: Type): AnyStruct? {
            return BlueCatfishCoin.resolveContractView(resourceType: nil, viewType: view)
        }

        access(all) view fun getSupportedVaultTypes(): {Type: Bool} {
            return {Type<@BlueCatfishCoin.Vault>(): true}
        }

        access(all) view fun isSupportedVaultType(type: Type): Bool {
            return type == Type<@BlueCatfishCoin.Vault>()
        }
        ...
        access(FungibleToken.Withdraw) fun withdraw(amount: UFix64): @BlueCatfishCoin.Vault {
            self.balance = self.balance - amount
            return <-create Vault(balance: amount)
        }

        access(all) fun deposit(from: @{FungibleToken.Vault}) {
            let vault <- from as! @BlueCatfishCoin.Vault
            self.balance = self.balance + vault.balance
            vault.balance = 0.0
            destroy vault
        }

        access(all) fun createEmptyVault(): @BlueCatfishCoin.Vault {
    ...
    access(all) resource Minter {
        ...
        access(all) fun mintForCatch(amount: UFix64, fishId: UInt64, angler: Address): @BlueCatfishCoin.Vault {
            pre {
                amount == 1.0: "Only 1 coin per verified catch"
                BlueCatfishCoin.fishNFTRegistry[fishId] == nil: "This NFT has already been used to mint a coin"
            }
            
            // Record this NFT as used
            BlueCatfishCoin.fishNFTRegistry[fishId] = true
        ...
        access(all) fun autoRegisterWithFishDEX() {
            if let fishDEXAddr = BlueCatfishCoin.fishDEXAddress {
                if !BlueCatfishCoin.isRegisteredWithFishDEX {
                    // Get reference to FishDEX coordinator
                    if let coordinatorRef = BlueCatfishCoin.account.capabilities.borrow<&FishDEXCoordinator>(
                        BlueCatfishCoin.FishDEXCoordinatorPublicPath
                    ) {
        ...
        access(all) fun mintBatch(recipients: {Address: UFix64}): @{Address: BlueCatfishCoin.Vault} {
            let vaults: @{Address: BlueCatfishCoin.Vault} <- {}
    ...
    access(all) resource MetadataAdmin {
        ...
        access(all) fun updateImageURL(newURL: String) {
            let oldURL = BlueCatfishCoin.speciesMetadata.imageURL ?? ""
            BlueCatfishCoin.speciesMetadata.setImageURL(newURL)
        ...
        access(all) fun updateDescription(newDescription: String) {
            let oldDescription = BlueCatfishCoin.speciesMetadata.description
            BlueCatfishCoin.speciesMetadata.setDescription(newDescription)
        ...
        access(all) fun updateHabitat(newHabitat: String) {
            let oldHabitat = BlueCatfishCoin.speciesMetadata.habitat ?? ""
            BlueCatfishCoin.speciesMetadata.setHabitat(newHabitat)
        ...
        access(all) fun updateAverageWeight(newWeight: UFix64) {
            let oldWeight = BlueCatfishCoin.speciesMetadata.averageWeight?.toString() ?? "0.0"
            BlueCatfishCoin.speciesMetadata.setAverageWeight(newWeight)
        ...
        access(all) fun updateAverageLength(newLength: UFix64) {
            let oldLength = BlueCatfishCoin.speciesMetadata.averageLength?.toString() ?? "0.0"
            BlueCatfishCoin.speciesMetadata.setAverageLength(newLength)
        ...
        access(all) fun updateRarityTier(newTier: UInt8) {
            ...
            let oldTier = BlueCatfishCoin.speciesMetadata.rarityTier?.toString() ?? "0"
            BlueCatfishCoin.speciesMetadata.setRarityTier(newTier)
        ...
        access(all) fun manuallySetFirstCatch(timestamp: UInt64, angler: Address) {
            pre {
                BlueCatfishCoin.speciesMetadata.firstCatchDate == nil: "First catch already recorded"
            }
            BlueCatfishCoin.speciesMetadata.setFirstCatchDate(timestamp)
        ...
        access(all) fun updateConservationStatus(newStatus: String) {
            let oldStatus = BlueCatfishCoin.speciesMetadata.globalConservationStatus ?? ""
            BlueCatfishCoin.speciesMetadata.setConservationStatus(newStatus)
        ...
        access(all) fun updateNativeRegions(newRegions: [String]) {
            var oldRegionsStr = "["
            for i, region in BlueCatfishCoin.speciesMetadata.nativeRegions {
                if i > 0 { oldRegionsStr = oldRegionsStr.concat(",") }
                oldRegionsStr = oldRegionsStr.concat(region)
            }
            oldRegionsStr = oldRegionsStr.concat("]")
            
            BlueCatfishCoin.speciesMetadata.setNativeRegions(newRegions)
        ...
        access(all) fun updateSeasonalPatterns(newPatterns: String) {
            let oldPatterns = BlueCatfishCoin.speciesMetadata.seasonalPatterns ?? ""
            BlueCatfishCoin.speciesMetadata.setSeasonalPatterns(newPatterns)
        ...
        access(all) fun updateRecordWeight(newRecord: UFix64) {
            let oldRecord = BlueCatfishCoin.speciesMetadata.recordWeight?.toString() ?? "0.0"
            BlueCatfishCoin.speciesMetadata.setRecordWeight(newRecord)
        ...
        access(all) fun updateRecordLength(newRecord: UFix64) {
            let oldRecord = BlueCatfishCoin.speciesMetadata.recordLength?.toString() ?? "0.0"
            BlueCatfishCoin.speciesMetadata.setRecordLength(newRecord)
        ...
        access(all) fun updateRecordWeightLocation(newLocation: String?) {
            let oldLocation = BlueCatfishCoin.speciesMetadata.recordWeightLocation ?? ""
            BlueCatfishCoin.speciesMetadata.setRecordWeightLocation(newLocation)
        ...
        access(all) fun updateRecordWeightDate(newDate: String?) {
            let oldDate = BlueCatfishCoin.speciesMetadata.recordWeightDate ?? ""
            BlueCatfishCoin.speciesMetadata.setRecordWeightDate(newDate)
        ...
        access(all) fun updateRecordLengthLocation(newLocation: String?) {
            let oldLocation = BlueCatfishCoin.speciesMetadata.recordLengthLocation ?? ""
            BlueCatfishCoin.speciesMetadata.setRecordLengthLocation(newLocation)
        ...
        access(all) fun updateRecordLengthDate(newDate: String?) {
            let oldDate = BlueCatfishCoin.speciesMetadata.recordLengthDate ?? ""
            BlueCatfishCoin.speciesMetadata.setRecordLengthDate(newDate)
        ...
        access(all) fun approvePendingUpdate(index: Int) {
            pre {
                index >= 0 && index < BlueCatfishCoin.pendingUpdates.length: "Invalid update index"
            }
            let update = BlueCatfishCoin.pendingUpdates.remove(at: index)
        ...
        access(all) fun rejectPendingUpdate(index: Int) {
            ...
            BlueCatfishCoin.pendingUpdates.remove(at: index)
        }
        
        access(all) fun clearAllPendingUpdates() {
            BlueCatfishCoin.pendingUpdates = []
        }
        
        // ENHANCED ADMIN FUNCTIONS WITH VALIDATION
        access(all) fun updateRarityTierValidated(newTier: UInt8) {
            pre {
                BlueCatfishCoin.validateRating(newTier): "Invalid rarity tier (must be 1-10)"
            }
            let oldTier = BlueCatfishCoin.speciesMetadata.rarityTier?.toString() ?? "0"
            BlueCatfishCoin.speciesMetadata.setRarityTier(newTier)
            emit MetadataUpdated(field: "rarityTier", oldValue: oldTier, newValue: newTier.toString())
        ...
        access(all) fun updateConservationStatusValidated(newStatus: String) {
            pre {
                BlueCatfishCoin.validateConservationStatus(newStatus): "Invalid conservation status"
            }
            let oldStatus = BlueCatfishCoin.speciesMetadata.globalConservationStatus ?? ""
            BlueCatfishCoin.speciesMetadata.setConservationStatus(newStatus)
            emit MetadataUpdated(field: "globalConservationStatus", oldValue: oldStatus, newValue: newStatus)
        ...
        access(all) fun updateFightRatingValidated(newRating: UInt8) {
            pre {
                BlueCatfishCoin.validateRating(newRating): "Fight rating must be 1-10"
            }
            let oldRating = BlueCatfishCoin.speciesMetadata.fightRating?.toString() ?? "0"
            BlueCatfishCoin.speciesMetadata.setFightRating(newRating)
        ...
        access(all) fun archiveCurrentYear() {
            let currentYear = BlueCatfishCoin.currentMetadataYear
            BlueCatfishCoin.metadataHistory[currentYear] = BlueCatfishCoin.speciesMetadata
            emit YearlyMetadataCreated(year: currentYear)
        }
        
        access(all) fun updateToNewYear(_ newYear: UInt64) {
            pre {
                newYear > BlueCatfishCoin.currentMetadataYear: "New year must be greater than current year"
            }
            // Archive current year data
            self.archiveCurrentYear()
            
            let oldYear = BlueCatfishCoin.currentMetadataYear
            BlueCatfishCoin.currentMetadataYear = newYear
            emit MetadataYearUpdated(oldYear: oldYear, newYear: newYear)
        }
    }

    // Public functions
    access(all) fun createEmptyVault(vaultType: Type): @BlueCatfishCoin.Vault {
        pre {
            vaultType == Type<@BlueCatfishCoin.Vault>(): "Vault type mismatch"
        }
        return <-create Vault(balance: 0.0)
    }
    ...
    // NOTE: BlueCatfish coins represent permanent historical catch records
    ...
    access(all) view fun getVaultBalance(vaultRef: &BlueCatfishCoin.Vault): UFix64 {
        return vaultRef.balance
    }
    
    access(all) view fun canWithdraw(vaultRef: &BlueCatfishCoin.Vault, amount: UFix64): Bool {
    ...
    access(all) view fun isValidVaultType(vaultType: Type): Bool {
        return vaultType == Type<@BlueCatfishCoin.Vault>()
    }
    ...
    init() {
        ...
        self.speciesMetadata = SpeciesMetadata(
            // IMMUTABLE CORE FIELDS
            speciesCode: "ICTALURUS_FURCATUS",
            ticker: "ICTFUR",
            ...
            // BASIC DESCRIPTIVE FIELDS
            commonName: "BlueCatfish",
            habitat: "Large rivers, reservoirs, and deep pools with strong currents",
            averageWeight: 25.0,
            averageLength: 32.0,
            imageURL: "https://derby.fish/images/species/BlueCatfish.jpg",
            description: "Blue Catfish are the largest freshwater fish in North America, known for their massive size potential and excellent table fare. They have distinctive blue-gray coloration and deeply forked tails.",
            firstCatchDate: nil,
            rarityTier: 4,
            ...
            regionalPopulations: {
                "Mississippi River System": RegionalPopulation(
                    populationTrend: "Stable",
                    threats: ["Dam Construction", "Habitat Modification", "Water Quality"],
                    protectedAreas: ["Upper Mississippi River National Wildlife Refuge"],
                    estimatedPopulation: nil
                ),
                "Chesapeake Bay": RegionalPopulation(
                    populationTrend: "Increasing",
                    threats: ["Invasive Species Impact", "Overpopulation"],
                    protectedAreas: ["Chesapeake Bay National Estuarine Research Reserve"],
                    estimatedPopulation: nil
                ),
                "Red River System": RegionalPopulation(
                    populationTrend: "Stable",
                    threats: ["Agricultural Runoff", "Habitat Loss", "Climate Change"],
                    protectedAreas: ["Red River National Wildlife Refuge"],
                    estimatedPopulation: nil
                )
            },
            ...
            // BIOLOGICAL INTELLIGENCE
            lifespan: 30.0,
            diet: "Fish, crayfish, mussels, insects, aquatic invertebrates, carrion",
            predators: ["Large alligators", "Humans", "Juvenile predation by various fish"],
            temperatureRange: "60-85°F (optimal 75-82°F)",
            depthRange: "10-50 feet",
            spawningAge: 5.0,
            spawningBehavior: "Spawns in cavities, males guard nests, prefers rocky or hard substrates",
            migrationPattern: "Moves upstream to spawn, follows food sources and thermal layers",
            waterQualityNeeds: "pH 6.5-8.5, high dissolved oxygen, tolerates turbidity",
            
            // GEOGRAPHIC & HABITAT
            nativeRegions: ["Mississippi River Basin", "Missouri River", "Ohio River", "Red River"],
            currentRange: ["Central and Eastern US", "Introduced to Mid-Atlantic", "Expanding Range"],
            waterTypes: ["Large River", "Reservoir", "Deep Pool", "Main Channel"],
            invasiveStatus: "Native in some areas, Invasive in others",
            
            // ECONOMIC & COMMERCIAL
            regionalCommercialValue: {
                "Mississippi River": 6.00,
                "Chesapeake Bay": 8.00,
                "Tennessee River": 5.50,
                "Red River": 4.50
            },
            tourismValue: 8,
            ecosystemRole: "Apex Predator, Top Consumer",
            culturalSignificance: "Trophy fishing target, important to commercial fishing industry",
            
            // ANGLING & RECREATIONAL
            bestBaits: ["Cut bait", "Live bait", "Stink baits", "Chicken liver", "Worms"],
            fightRating: 8,
            culinaryRating: 8,
            catchDifficulty: 6,
            seasonalAvailability: "Best in spring and fall, active year-round in warm waters",
            bestTechniques: ["Bottom fishing", "Drift fishing", "Jugging", "Trotlining", "Still fishing"],
            
            // REGULATORY
            regionalRegulations: {
                "Mississippi": RegionalRegulations(
                    sizeLimit: 30.0,
                    bagLimit: 10,
                    closedSeasons: [],
                    specialRegulations: "Special length limits on some waters",
                    licenseRequired: true
                ),
                "Tennessee": RegionalRegulations(
                    sizeLimit: 34.0,
                    bagLimit: 5,
                    closedSeasons: [],
                    specialRegulations: "Trophy management on some waters",
                    licenseRequired: true
                ),
                "Virginia": RegionalRegulations(
                    sizeLimit: 32.0,
                    bagLimit: 4,
                    closedSeasons: [],
                    specialRegulations: "Invasive species - no live transport",
                    licenseRequired: true
                )
            },
            
            // PHYSICAL & BEHAVIORAL
            physicalDescription: "Blue-gray coloration, deeply forked tail, barbels around mouth, smooth skin",
            behaviorTraits: "Bottom feeders, opportunistic hunters, strong swimmers, structure-oriented",
            seasonalPatterns: "Spring spawning migration, summer deep water, fall feeding, winter dormancy",
            
            // RECORDS & ACHIEVEMENTS
            recordWeight: 143.0,
            recordWeightLocation: "Kerr Lake, Virginia",
            recordWeightDate: "June 18, 2011",
            recordLength: 57.0,
            recordLengthLocation: "Missouri River, Missouri",
            recordLengthDate: "May 22, 2010",
            
            // RESEARCH & SCIENTIFIC
            researchPriority: 7,
            geneticMarkers: "Studies on population genetics and hybridization with channel catfish",
            studyPrograms: ["US Fish and Wildlife Service", "State fisheries agencies", "University research programs"],
        ...
        self.VaultStoragePath = /storage/BlueCatfishCoinVault
        self.VaultPublicPath = /public/BlueCatfishCoinReceiver
        self.MinterStoragePath = /storage/BlueCatfishCoinMinter
        self.MetadataAdminStoragePath = /storage/BlueCatfishCoinMetadataAdmin
        self.FishDEXCoordinatorStoragePath = /storage/BlueCatfishCoinFishDEXCoordinator
        self.FishDEXCoordinatorPublicPath = /public/BlueCatfishCoinFishDEXCoordinator
        ...
        let cap = self.account.capabilities.storage.issue<&BlueCatfishCoin.Vault>(self.VaultStoragePath)
