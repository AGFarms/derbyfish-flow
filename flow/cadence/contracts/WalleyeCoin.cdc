...
access(all) contract WalleyeCoin: FungibleToken {
        ...
        switch viewType {
            ...
            case Type<FungibleTokenMetadataViews.FTVaultData>():
                return FungibleTokenMetadataViews.FTVaultData(
                    ...
                    receiverLinkedType: Type<&WalleyeCoin.Vault>(),
                    metadataLinkedType: Type<&WalleyeCoin.Vault>(),
                    createEmptyVaultFunction: (fun(): @{FungibleToken.Vault} {
                        return <-WalleyeCoin.createEmptyVault(vaultType: Type<@WalleyeCoin.Vault>())
                    })
                )
    ...
    access(all) resource FishDEXCoordinator: SpeciesCoinPublic {
        ...
        access(all) view fun getSpeciesCode(): String {
            return WalleyeCoin.speciesMetadata.speciesCode
        }
        
        access(all) view fun getTicker(): String {
            return WalleyeCoin.speciesMetadata.ticker
        }
        
        access(all) view fun getCommonName(): String {
            return WalleyeCoin.speciesMetadata.commonName
        }
        
        access(all) view fun getScientificName(): String {
            return WalleyeCoin.speciesMetadata.scientificName
        }
        
        access(all) view fun getFamily(): String {
            return WalleyeCoin.speciesMetadata.family
        }
        
        access(all) view fun getTotalSupply(): UFix64 {
            return WalleyeCoin.totalSupply
        }
        ...
        access(all) view fun getBasicRegistryInfo(): {String: AnyStruct} {
            return {
                ...
                "contractAddress": WalleyeCoin.account.address,
                "dataYear": WalleyeCoin.speciesMetadata.dataYear,
                "conservationStatus": WalleyeCoin.speciesMetadata.globalConservationStatus,
                "rarityTier": WalleyeCoin.speciesMetadata.rarityTier,
                "isRegistered": WalleyeCoin.isRegisteredWithFishDEX
            }
        }
        ...
        access(all) fun redeemCatchNFT(fishData: {String: AnyStruct}, angler: Address): @WalleyeCoin.Vault {
            ...
            if WalleyeCoin.totalSupply == 0.0 && WalleyeCoin.speciesMetadata.firstCatchDate == nil {
                WalleyeCoin.speciesMetadata.setFirstCatchDate(UInt64(getCurrentBlock().timestamp))
            ...
            WalleyeCoin.totalSupply = WalleyeCoin.totalSupply + amount
        ...
        access(all) fun registerWithFishDEX(fishDEXAddress: Address) {
            pre {
                !WalleyeCoin.isRegisteredWithFishDEX: "Already registered with FishDEX"
            }
            ...
            WalleyeCoin.fishDEXAddress = fishDEXAddress
            WalleyeCoin.isRegisteredWithFishDEX = true
        ...
        access(all) fun updateFishDEXAddress(newAddress: Address) {
            let oldAddress = WalleyeCoin.fishDEXAddress
            WalleyeCoin.fishDEXAddress = newAddress
            WalleyeCoin.isRegisteredWithFishDEX = false // Reset registration status
    ...
    access(all) resource Vault: FungibleToken.Vault {
        ...
        access(contract) fun burnCallback() {
            if self.balance > 0.0 {
                WalleyeCoin.totalSupply = WalleyeCoin.totalSupply - self.balance
        ...
        access(all) view fun getViews(): [Type] {
            return WalleyeCoin.getContractViews(resourceType: nil)
        }

        access(all) fun resolveView(_ view: Type): AnyStruct? {
            return WalleyeCoin.resolveContractView(resourceType: nil, viewType: view)
        }

        access(all) view fun getSupportedVaultTypes(): {Type: Bool} {
            return {Type<@WalleyeCoin.Vault>(): true}
        }

        access(all) view fun isSupportedVaultType(type: Type): Bool {
            return type == Type<@WalleyeCoin.Vault>()
        }
        ...
        access(FungibleToken.Withdraw) fun withdraw(amount: UFix64): @WalleyeCoin.Vault {
            self.balance = self.balance - amount
            return <-create Vault(balance: amount)
        }

        access(all) fun deposit(from: @{FungibleToken.Vault}) {
            let vault <- from as! @WalleyeCoin.Vault
            self.balance = self.balance + vault.balance
            vault.balance = 0.0
            destroy vault
        }

        access(all) fun createEmptyVault(): @WalleyeCoin.Vault {
    ...
    access(all) resource Minter {
        ...
        access(all) fun autoRegisterWithFishDEX() {
            if let fishDEXAddr = WalleyeCoin.fishDEXAddress {
                if !WalleyeCoin.isRegisteredWithFishDEX {
                    // Get reference to FishDEX coordinator
                    if let coordinatorRef = WalleyeCoin.account.capabilities.borrow<&FishDEXCoordinator>(
        ...
        access(all) fun mintBatch(recipients: {Address: UFix64}): @{Address: WalleyeCoin.Vault} {
    ...
    access(all) resource MetadataAdmin {
        ...
        access(all) fun updateImageURL(newURL: String) {
            let oldURL = WalleyeCoin.speciesMetadata.imageURL ?? ""
            WalleyeCoin.speciesMetadata.setImageURL(newURL)
        ...
        access(all) fun updateDescription(newDescription: String) {
            let oldDescription = WalleyeCoin.speciesMetadata.description
            WalleyeCoin.speciesMetadata.setDescription(newDescription)
        ...
        access(all) fun updateHabitat(newHabitat: String) {
            let oldHabitat = WalleyeCoin.speciesMetadata.habitat ?? ""
            WalleyeCoin.speciesMetadata.setHabitat(newHabitat)
        ...
        access(all) fun updateAverageWeight(newWeight: UFix64) {
            let oldWeight = WalleyeCoin.speciesMetadata.averageWeight?.toString() ?? "0.0"
            WalleyeCoin.speciesMetadata.setAverageWeight(newWeight)
        ...
        access(all) fun updateAverageLength(newLength: UFix64) {
            let oldLength = WalleyeCoin.speciesMetadata.averageLength?.toString() ?? "0.0"
            WalleyeCoin.speciesMetadata.setAverageLength(newLength)
        ...
        access(all) fun updateRarityTier(newTier: UInt8) {
            ...
            let oldTier = WalleyeCoin.speciesMetadata.rarityTier?.toString() ?? "0"
            WalleyeCoin.speciesMetadata.setRarityTier(newTier)
        ...
        access(all) fun manuallySetFirstCatch(timestamp: UInt64, angler: Address) {
            pre {
                WalleyeCoin.speciesMetadata.firstCatchDate == nil: "First catch already recorded"
            }
            WalleyeCoin.speciesMetadata.setFirstCatchDate(timestamp)
        ...
        access(all) fun updateConservationStatus(newStatus: String) {
            let oldStatus = WalleyeCoin.speciesMetadata.globalConservationStatus ?? ""
            WalleyeCoin.speciesMetadata.setConservationStatus(newStatus)
        ...
        access(all) fun updateSeasonalPatterns(newPatterns: String) {
            let oldPatterns = WalleyeCoin.speciesMetadata.seasonalPatterns ?? ""
            WalleyeCoin.speciesMetadata.setSeasonalPatterns(newPatterns)
        ...
        access(all) fun updateRecordWeight(newRecord: UFix64) {
            let oldRecord = WalleyeCoin.speciesMetadata.recordWeight?.toString() ?? "0.0"
            WalleyeCoin.speciesMetadata.setRecordWeight(newRecord)
        ...
        access(all) fun updateRecordLength(newRecord: UFix64) {
            let oldRecord = WalleyeCoin.speciesMetadata.recordLength?.toString() ?? "0.0"
            WalleyeCoin.speciesMetadata.setRecordLength(newRecord)
        ...
        access(all) fun updateRecordWeightLocation(newLocation: String?) {
            let oldLocation = WalleyeCoin.speciesMetadata.recordWeightLocation ?? ""
            WalleyeCoin.speciesMetadata.setRecordWeightLocation(newLocation)
        ...
        access(all) fun updateRecordWeightDate(newDate: String?) {
            let oldDate = WalleyeCoin.speciesMetadata.recordWeightDate ?? ""
            WalleyeCoin.speciesMetadata.setRecordWeightDate(newDate)
        ...
        access(all) fun updateRecordLengthLocation(newLocation: String?) {
            let oldLocation = WalleyeCoin.speciesMetadata.recordLengthLocation ?? ""
            WalleyeCoin.speciesMetadata.setRecordLengthLocation(newLocation)
        ...
        access(all) fun updateRecordLengthDate(newDate: String?) {
            let oldDate = WalleyeCoin.speciesMetadata.recordLengthDate ?? ""
            WalleyeCoin.speciesMetadata.setRecordLengthDate(newDate)
        ...
        access(all) fun approvePendingUpdate(index: Int) {
            pre {
                index >= 0 && index < WalleyeCoin.pendingUpdates.length: "Invalid update index"
            }
            let update = WalleyeCoin.pendingUpdates.remove(at: index)
        ...
        access(all) fun rejectPendingUpdate(index: Int) {
            ...
            WalleyeCoin.pendingUpdates.remove(at: index)
        }
        ...
        access(all) fun updateRarityTierValidated(newTier: UInt8) {
            pre {
                WalleyeCoin.validateRating(newTier): "Invalid rarity tier (must be 1-10)"
            }
            let oldTier = WalleyeCoin.speciesMetadata.rarityTier?.toString() ?? "0"
            WalleyeCoin.speciesMetadata.setRarityTier(newTier)
            emit MetadataUpdated(field: "rarityTier", oldValue: oldTier, newValue: newTier.toString())
        ...
        access(all) fun updateConservationStatusValidated(newStatus: String) {
            pre {
                WalleyeCoin.validateConservationStatus(newStatus): "Invalid conservation status"
            }
            let oldStatus = WalleyeCoin.speciesMetadata.globalConservationStatus ?? ""
            WalleyeCoin.speciesMetadata.setConservationStatus(newStatus)
            emit MetadataUpdated(field: "globalConservationStatus", oldValue: oldStatus, newValue: newStatus)
        ...
        access(all) fun updateFightRatingValidated(newRating: UInt8) {
            pre {
                WalleyeCoin.validateRating(newRating): "Fight rating must be 1-10"
            }
            let oldRating = WalleyeCoin.speciesMetadata.fightRating?.toString() ?? "0"
            WalleyeCoin.speciesMetadata.setFightRating(newRating)
        ...
        access(all) fun archiveCurrentYear() {
            let currentYear = WalleyeCoin.currentMetadataYear
            WalleyeCoin.metadataHistory[currentYear] = WalleyeCoin.speciesMetadata
            emit YearlyMetadataCreated(year: currentYear)
        }
        
        access(all) fun updateToNewYear(_ newYear: UInt64) {
            pre {
                newYear > WalleyeCoin.currentMetadataYear: "New year must be greater than current year"
            }
            // Archive current year data
            self.archiveCurrentYear()
            
            let oldYear = WalleyeCoin.currentMetadataYear
            WalleyeCoin.currentMetadataYear = newYear
            emit MetadataYearUpdated(oldYear: oldYear, newYear: newYear)
        }
    }

    // Public functions
    access(all) fun createEmptyVault(vaultType: Type): @WalleyeCoin.Vault {
        pre {
            vaultType == Type<@WalleyeCoin.Vault>(): "Vault type mismatch"
        }
        return <-create Vault(balance: 0.0)
    }
    ...
    // NOTE: Walleye coins represent permanent historical catch records
    ...
    access(all) view fun getVaultBalance(vaultRef: &WalleyeCoin.Vault): UFix64 {
        return vaultRef.balance
    }
    
    access(all) view fun canWithdraw(vaultRef: &WalleyeCoin.Vault, amount: UFix64): Bool {
    ...
    access(all) view fun isValidVaultType(vaultType: Type): Bool {
        return vaultType == Type<@WalleyeCoin.Vault>()
    }
    ...
    init() {
        ...
        self.speciesMetadata = SpeciesMetadata(
            ...
            // BASIC DESCRIPTIVE FIELDS
            commonName: "Walleye",
            habitat: "Cool freshwater lakes, rivers, and reservoirs",
            averageWeight: 2.5,
            averageLength: 16.0,
            imageURL: "https://derby.fish/images/species/walleye.jpg",
            description: "Walleye are prized freshwater gamefish known for their excellent table fare and challenging nocturnal feeding habits. They have distinctive glassy eyes that reflect light, giving them their name.",
            firstCatchDate: nil,
            rarityTier: 3,
            ...
            regionalPopulations: {
                "Great Lakes": RegionalPopulation(
                    ...
                    threats: ["Climate Change", "Zebra Mussels", "Overfishing"],
                    protectedAreas: ["Great Lakes National Marine Sanctuaries"],
                    estimatedPopulation: 50000000
                ),
                ...
                "Mississippi River System": RegionalPopulation(
                    ...
                    threats: ["Dam Construction", "Agricultural Runoff", "Habitat Fragmentation"],
            ...
            // BIOLOGICAL INTELLIGENCE
            lifespan: 20.0,
            diet: "Yellow perch, minnows, crayfish, insects, leeches",
            predators: ["Northern Pike", "Muskellunge", "Large Lake Trout", "Cormorants", "Humans"],
            temperatureRange: "32-75°F (optimal 60-70°F)",
            depthRange: "10-100 feet",
            spawningAge: 4.0,
            spawningBehavior: "Spawns in shallow rocky areas in early spring, scatters eggs over gravel beds",
            migrationPattern: "Moves to deeper water in summer, shallow areas in spring and fall",
            waterQualityNeeds: "pH 6.0-8.0, high dissolved oxygen, clear water preferred",
            
            // GEOGRAPHIC & HABITAT
            nativeRegions: ["Great Lakes", "Canadian Shield", "Northern Great Plains"],
            currentRange: ["Northern US", "Southern Canada", "Introduced to Western US"],
            waterTypes: ["Lake", "River", "Reservoir", "Large River Systems"],
            ...
            tourismValue: 9,
            ecosystemRole: "Apex Predator (smaller waters), Key Predator",
            culturalSignificance: "State fish of Minnesota, major cultural importance to Ojibwe and other indigenous peoples",
            
            // ANGLING & RECREATIONAL
            bestBaits: ["Jigs with minnows", "Crawler harnesses", "Crankbaits", "Live bait rigs", "Leeches"],
            fightRating: 5,
            culinaryRating: 10,
            catchDifficulty: 7,
            seasonalAvailability: "Best at dawn/dusk and night, spring and fall peak",
            bestTechniques: ["Trolling", "Jigging", "Live bait fishing", "Casting crankbaits", "Slip bobber fishing"],
            ...
            regionalRegulations: {
                "Minnesota": RegionalRegulations(
                    ...
                    specialRegulations: "One fish over 20 inches on many lakes",
                ...
                "Great Lakes": RegionalRegulations(
                    sizeLimit: 15.0,
                    bagLimit: 5,
                    closedSeasons: [],
            ...
            // PHYSICAL & BEHAVIORAL
            physicalDescription: "Olive-gold body with white belly, large glassy eyes, white-tipped fins, canine teeth",
            behaviorTraits: "Nocturnal feeders, schooling in open water, structure-oriented, light-sensitive",
            seasonalPatterns: "Spring shallow spawning, summer deep water, fall feeding migration, winter dormancy",
            
            // RECORDS & ACHIEVEMENTS
            recordWeight: 25.0,
            recordWeightLocation: "Old Hickory Lake, Tennessee",
            recordWeightDate: "August 2, 1960",
            recordLength: 42.0,
            recordLengthLocation: "Greers Ferry Lake, Arkansas",
            recordLengthDate: "April 14, 1982",
            
            // RESEARCH & SCIENTIFIC
            researchPriority: 8,
            geneticMarkers: "Extensive genetic studies for stock enhancement programs",
            studyPrograms: ["Minnesota DNR Walleye Research", "Great Lakes Fishery Commission", "Ontario Ministry Research"],
            ...
            additionalMetadata: {
                ...
                "state_fish": "Minnesota",
                "nickname": "Marble Eyes"
            }
        )

        // Set storage paths using common name format
        self.VaultStoragePath = /storage/WalleyeCoinVault
        self.VaultPublicPath = /public/WalleyeCoinReceiver
        self.MinterStoragePath = /storage/WalleyeCoinMinter
        self.MetadataAdminStoragePath = /storage/WalleyeCoinMetadataAdmin
        self.FishDEXCoordinatorStoragePath = /storage/WalleyeCoinFishDEXCoordinator
        self.FishDEXCoordinatorPublicPath = /public/WalleyeCoinFishDEXCoordinator
        ...
        let cap = self.account.capabilities.storage.issue<&WalleyeCoin.Vault>(self.VaultStoragePath)
