...
access(all) contract MuskellungeCoin: FungibleToken {
    ...
    access(all) resource interface SpeciesCoinPublic {
        ...
        access(all) fun redeemCatchNFT(fishData: {String: AnyStruct}, angler: Address): @MuskellungeCoin.Vault
    }
        ...
        switch viewType {
            ...
            case Type<FungibleTokenMetadataViews.FTVaultData>():
                return FungibleTokenMetadataViews.FTVaultData(
                    ...
                    receiverLinkedType: Type<&MuskellungeCoin.Vault>(),
                    metadataLinkedType: Type<&MuskellungeCoin.Vault>(),
                    createEmptyVaultFunction: (fun(): @{FungibleToken.Vault} {
                        return <-MuskellungeCoin.createEmptyVault(vaultType: Type<@MuskellungeCoin.Vault>())
                    })
                )
    ...
    access(all) resource FishDEXCoordinator: SpeciesCoinPublic {
        ...
        access(all) view fun getSpeciesCode(): String {
            return MuskellungeCoin.speciesMetadata.speciesCode
        }
        
        access(all) view fun getTicker(): String {
            return MuskellungeCoin.speciesMetadata.ticker
        }
        
        access(all) view fun getCommonName(): String {
            return MuskellungeCoin.speciesMetadata.commonName
        }
        
        access(all) view fun getScientificName(): String {
            return MuskellungeCoin.speciesMetadata.scientificName
        }
        
        access(all) view fun getFamily(): String {
            return MuskellungeCoin.speciesMetadata.family
        }
        
        access(all) view fun getTotalSupply(): UFix64 {
            return MuskellungeCoin.totalSupply
        }
        ...
        access(all) view fun getBasicRegistryInfo(): {String: AnyStruct} {
            return {
                ...
                "contractAddress": MuskellungeCoin.account.address,
                "dataYear": MuskellungeCoin.speciesMetadata.dataYear,
                "conservationStatus": MuskellungeCoin.speciesMetadata.globalConservationStatus,
                "rarityTier": MuskellungeCoin.speciesMetadata.rarityTier,
                "isRegistered": MuskellungeCoin.isRegisteredWithFishDEX
            }
        }
        ...
        access(all) fun redeemCatchNFT(fishData: {String: AnyStruct}, angler: Address): @MuskellungeCoin.Vault {
            ...
            if let nftId = fishNFTId {
                assert(
                    MuskellungeCoin.fishNFTRegistry[nftId] == nil,
                    message: "This NFT has already been redeemed for a coin"
                )
                // Record this NFT as redeemed
                MuskellungeCoin.fishNFTRegistry[nftId] = true
            }
            
            // Auto-record first catch if this is the very first mint
            if MuskellungeCoin.totalSupply == 0.0 && MuskellungeCoin.speciesMetadata.firstCatchDate == nil {
                MuskellungeCoin.speciesMetadata.setFirstCatchDate(UInt64(getCurrentBlock().timestamp))
            ...
            MuskellungeCoin.totalSupply = MuskellungeCoin.totalSupply + amount
        ...
        access(all) fun registerWithFishDEX(fishDEXAddress: Address) {
            pre {
                !MuskellungeCoin.isRegisteredWithFishDEX: "Already registered with FishDEX"
            }
            ...
            MuskellungeCoin.fishDEXAddress = fishDEXAddress
            MuskellungeCoin.isRegisteredWithFishDEX = true
        ...
        access(all) fun updateFishDEXAddress(newAddress: Address) {
            let oldAddress = MuskellungeCoin.fishDEXAddress
            MuskellungeCoin.fishDEXAddress = newAddress
            MuskellungeCoin.isRegisteredWithFishDEX = false // Reset registration status
    ...
    access(all) resource Vault: FungibleToken.Vault {
        ...
        access(contract) fun burnCallback() {
            if self.balance > 0.0 {
                MuskellungeCoin.totalSupply = MuskellungeCoin.totalSupply - self.balance
        ...
        access(all) view fun getViews(): [Type] {
            return MuskellungeCoin.getContractViews(resourceType: nil)
        }

        access(all) fun resolveView(_ view: Type): AnyStruct? {
            return MuskellungeCoin.resolveContractView(resourceType: nil, viewType: view)
        }

        access(all) view fun getSupportedVaultTypes(): {Type: Bool} {
            return {Type<@MuskellungeCoin.Vault>(): true}
        }

        access(all) view fun isSupportedVaultType(type: Type): Bool {
            return type == Type<@MuskellungeCoin.Vault>()
        }
        ...
        access(FungibleToken.Withdraw) fun withdraw(amount: UFix64): @MuskellungeCoin.Vault {
            self.balance = self.balance - amount
            return <-create Vault(balance: amount)
        }

        access(all) fun deposit(from: @{FungibleToken.Vault}) {
            let vault <- from as! @MuskellungeCoin.Vault
            self.balance = self.balance + vault.balance
            vault.balance = 0.0
            destroy vault
        }

        access(all) fun createEmptyVault(): @MuskellungeCoin.Vault {
    ...
    access(all) resource Minter {
        ...
        access(all) fun mintForCatch(amount: UFix64, fishId: UInt64, angler: Address): @MuskellungeCoin.Vault {
            pre {
                amount == 1.0: "Only 1 coin per verified catch"
                MuskellungeCoin.fishNFTRegistry[fishId] == nil: "This NFT has already been used to mint a coin"
            }
            
            // Record this NFT as used
            MuskellungeCoin.fishNFTRegistry[fishId] = true
        ...
        access(all) fun autoRegisterWithFishDEX() {
            if let fishDEXAddr = MuskellungeCoin.fishDEXAddress {
                if !MuskellungeCoin.isRegisteredWithFishDEX {
                    // Get reference to FishDEX coordinator
                    if let coordinatorRef = MuskellungeCoin.account.capabilities.borrow<&FishDEXCoordinator>(
                        MuskellungeCoin.FishDEXCoordinatorPublicPath
                    ) {
        ...
        access(all) fun mintBatch(recipients: {Address: UFix64}): @{Address: MuskellungeCoin.Vault} {
            let vaults: @{Address: MuskellungeCoin.Vault} <- {}
    ...
    access(all) resource MetadataAdmin {
        ...
        access(all) fun updateImageURL(newURL: String) {
            let oldURL = MuskellungeCoin.speciesMetadata.imageURL ?? ""
            MuskellungeCoin.speciesMetadata.setImageURL(newURL)
        ...
        access(all) fun updateDescription(newDescription: String) {
            let oldDescription = MuskellungeCoin.speciesMetadata.description
            MuskellungeCoin.speciesMetadata.setDescription(newDescription)
        ...
        access(all) fun updateHabitat(newHabitat: String) {
            let oldHabitat = MuskellungeCoin.speciesMetadata.habitat ?? ""
            MuskellungeCoin.speciesMetadata.setHabitat(newHabitat)
        ...
        access(all) fun updateAverageWeight(newWeight: UFix64) {
            let oldWeight = MuskellungeCoin.speciesMetadata.averageWeight?.toString() ?? "0.0"
            MuskellungeCoin.speciesMetadata.setAverageWeight(newWeight)
        ...
        access(all) fun updateAverageLength(newLength: UFix64) {
            let oldLength = MuskellungeCoin.speciesMetadata.averageLength?.toString() ?? "0.0"
            MuskellungeCoin.speciesMetadata.setAverageLength(newLength)
        ...
        access(all) fun updateRarityTier(newTier: UInt8) {
            ...
            let oldTier = MuskellungeCoin.speciesMetadata.rarityTier?.toString() ?? "0"
            MuskellungeCoin.speciesMetadata.setRarityTier(newTier)
        ...
        access(all) fun manuallySetFirstCatch(timestamp: UInt64, angler: Address) {
            pre {
                MuskellungeCoin.speciesMetadata.firstCatchDate == nil: "First catch already recorded"
            }
            MuskellungeCoin.speciesMetadata.setFirstCatchDate(timestamp)
        ...
        access(all) fun updateConservationStatus(newStatus: String) {
            let oldStatus = MuskellungeCoin.speciesMetadata.globalConservationStatus ?? ""
            MuskellungeCoin.speciesMetadata.setConservationStatus(newStatus)
        ...
        access(all) fun updateNativeRegions(newRegions: [String]) {
            var oldRegionsStr = "["
            for i, region in MuskellungeCoin.speciesMetadata.nativeRegions {
                if i > 0 { oldRegionsStr = oldRegionsStr.concat(",") }
                oldRegionsStr = oldRegionsStr.concat(region)
            }
            oldRegionsStr = oldRegionsStr.concat("]")
            
            MuskellungeCoin.speciesMetadata.setNativeRegions(newRegions)
        ...
        access(all) fun updateSeasonalPatterns(newPatterns: String) {
            let oldPatterns = MuskellungeCoin.speciesMetadata.seasonalPatterns ?? ""
            MuskellungeCoin.speciesMetadata.setSeasonalPatterns(newPatterns)
        ...
        access(all) fun updateRecordWeight(newRecord: UFix64) {
            let oldRecord = MuskellungeCoin.speciesMetadata.recordWeight?.toString() ?? "0.0"
            MuskellungeCoin.speciesMetadata.setRecordWeight(newRecord)
        ...
        access(all) fun updateRecordLength(newRecord: UFix64) {
            let oldRecord = MuskellungeCoin.speciesMetadata.recordLength?.toString() ?? "0.0"
            MuskellungeCoin.speciesMetadata.setRecordLength(newRecord)
        ...
        access(all) fun updateRecordWeightLocation(newLocation: String?) {
            let oldLocation = MuskellungeCoin.speciesMetadata.recordWeightLocation ?? ""
            MuskellungeCoin.speciesMetadata.setRecordWeightLocation(newLocation)
        ...
        access(all) fun updateRecordWeightDate(newDate: String?) {
            let oldDate = MuskellungeCoin.speciesMetadata.recordWeightDate ?? ""
            MuskellungeCoin.speciesMetadata.setRecordWeightDate(newDate)
        ...
        access(all) fun updateRecordLengthLocation(newLocation: String?) {
            let oldLocation = MuskellungeCoin.speciesMetadata.recordLengthLocation ?? ""
            MuskellungeCoin.speciesMetadata.setRecordLengthLocation(newLocation)
        ...
        access(all) fun updateRecordLengthDate(newDate: String?) {
            let oldDate = MuskellungeCoin.speciesMetadata.recordLengthDate ?? ""
            MuskellungeCoin.speciesMetadata.setRecordLengthDate(newDate)
        ...
        access(all) fun approvePendingUpdate(index: Int) {
            pre {
                index >= 0 && index < MuskellungeCoin.pendingUpdates.length: "Invalid update index"
            }
            let update = MuskellungeCoin.pendingUpdates.remove(at: index)
        ...
        access(all) fun rejectPendingUpdate(index: Int) {
            ...
            MuskellungeCoin.pendingUpdates.remove(at: index)
        }
        
        access(all) fun clearAllPendingUpdates() {
            MuskellungeCoin.pendingUpdates = []
        }
        
        // ENHANCED ADMIN FUNCTIONS WITH VALIDATION
        access(all) fun updateRarityTierValidated(newTier: UInt8) {
            pre {
                MuskellungeCoin.validateRating(newTier): "Invalid rarity tier (must be 1-10)"
            }
            let oldTier = MuskellungeCoin.speciesMetadata.rarityTier?.toString() ?? "0"
            MuskellungeCoin.speciesMetadata.setRarityTier(newTier)
            emit MetadataUpdated(field: "rarityTier", oldValue: oldTier, newValue: newTier.toString())
        ...
        access(all) fun updateConservationStatusValidated(newStatus: String) {
            pre {
                MuskellungeCoin.validateConservationStatus(newStatus): "Invalid conservation status"
            }
            let oldStatus = MuskellungeCoin.speciesMetadata.globalConservationStatus ?? ""
            MuskellungeCoin.speciesMetadata.setConservationStatus(newStatus)
            emit MetadataUpdated(field: "globalConservationStatus", oldValue: oldStatus, newValue: newStatus)
        ...
        access(all) fun updateFightRatingValidated(newRating: UInt8) {
            pre {
                MuskellungeCoin.validateRating(newRating): "Fight rating must be 1-10"
            }
            let oldRating = MuskellungeCoin.speciesMetadata.fightRating?.toString() ?? "0"
            MuskellungeCoin.speciesMetadata.setFightRating(newRating)
        ...
        access(all) fun archiveCurrentYear() {
            let currentYear = MuskellungeCoin.currentMetadataYear
            MuskellungeCoin.metadataHistory[currentYear] = MuskellungeCoin.speciesMetadata
            emit YearlyMetadataCreated(year: currentYear)
        }
        
        access(all) fun updateToNewYear(_ newYear: UInt64) {
            pre {
                newYear > MuskellungeCoin.currentMetadataYear: "New year must be greater than current year"
            }
            // Archive current year data
            self.archiveCurrentYear()
            
            let oldYear = MuskellungeCoin.currentMetadataYear
            MuskellungeCoin.currentMetadataYear = newYear
            emit MetadataYearUpdated(oldYear: oldYear, newYear: newYear)
        }
    }

    // Public functions
    access(all) fun createEmptyVault(vaultType: Type): @MuskellungeCoin.Vault {
        pre {
            vaultType == Type<@MuskellungeCoin.Vault>(): "Vault type mismatch"
        }
        return <-create Vault(balance: 0.0)
    }
    ...
    // NOTE: Muskellunge coins represent permanent historical catch records
    ...
    access(all) view fun getVaultBalance(vaultRef: &MuskellungeCoin.Vault): UFix64 {
        return vaultRef.balance
    }
    
    access(all) view fun canWithdraw(vaultRef: &MuskellungeCoin.Vault, amount: UFix64): Bool {
    ...
    access(all) view fun isValidVaultType(vaultType: Type): Bool {
        return vaultType == Type<@MuskellungeCoin.Vault>()
    }
    ...
    init() {
        ...
        self.speciesMetadata = SpeciesMetadata(
            ...
            habitat: "Large clear lakes, rivers, reservoirs with structure and vegetation",
            averageWeight: 20.0,
            averageLength: 36.0,
            imageURL: "https://derby.fish/images/species/Muskellunge.jpg",
            description: "Muskellunge are the ultimate freshwater trophy fish, known as the 'fish of 10,000 casts.' They are the largest member of the pike family and apex predators in their ecosystems, revered for their size, strength, and elusive nature.",
            firstCatchDate: nil,
            rarityTier: 5,
            ...
            regionalPopulations: {
                ...
                "Tennessee River System": RegionalPopulation(
                    populationTrend: "Recovering",
                    threats: ["Dam Construction", "Water Quality", "Habitat Fragmentation"],
            ...
            // BIOLOGICAL INTELLIGENCE
            lifespan: 30.0,
            diet: "Large fish, waterfowl, small mammals, suckers, pike, bass",
            predators: ["Humans", "Very large fish (rare)"],
            temperatureRange: "50-78°F (optimal 60-70°F)",
            depthRange: "0-60 feet",
            spawningAge: 6.0,
            spawningBehavior: "Spawns in shallow weedy areas in spring, broadcasts eggs over vegetation",
            migrationPattern: "Deep water in summer, shallow structure in spring and fall",
            waterQualityNeeds: "pH 6.0-8.0, high dissolved oxygen, clear water preferred",
            
            // GEOGRAPHIC & HABITAT
            nativeRegions: ["Great Lakes", "Canadian Shield", "Upper Mississippi", "Tennessee River"],
            currentRange: ["Northern US", "Southern Canada", "Introduced to select waters"],
            waterTypes: ["Lake", "River", "Reservoir", "Large Clear Waters"],
            ...
            culturalSignificance: "State fish of Wisconsin, legendary trophy fish, cultural icon",
            
            // ANGLING & RECREATIONAL
            bestBaits: ["Large lures", "Jerkbaits", "Spinnerbaits", "Live suckers", "Bucktails"],
            fightRating: 10,
            culinaryRating: 7,
            catchDifficulty: 10,
            seasonalAvailability: "Best in spring and fall, summer deeper water",
            bestTechniques: ["Trolling", "Casting large lures", "Figure-8", "Live bait fishing", "Fly fishing"],
            ...
            regionalRegulations: {
                "Minnesota": RegionalRegulations(
                    ...
                    specialRegulations: "Most lakes catch and release only",
                ...
                "Wisconsin": RegionalRegulations(
                    ...
                    specialRegulations: "Harvest tag required",
                    licenseRequired: true
                ),
                "Ontario": RegionalRegulations(
                    sizeLimit: 44.0,
                    bagLimit: 1,
                    closedSeasons: ["April 1 - June 15"],
                    specialRegulations: "Conservation license restrictions",
                    licenseRequired: true
                )
            },
            
            // PHYSICAL & BEHAVIORAL
            physicalDescription: "Largest pike family member, dark vertical bars or spots, pointed fins, large mouth with prominent teeth",
            behaviorTraits: "Solitary apex predator, ambush hunter, territorial, follows bait",
            seasonalPatterns: "Spring shallow spawning, summer deep structure, fall feeding, winter dormancy",
            
            // RECORDS & ACHIEVEMENTS
            recordWeight: 67.8,
            recordWeightLocation: "Lake Court Oreilles, Wisconsin",
            recordWeightDate: "July 24, 1949",
            recordLength: 60.25,
            recordLengthLocation: "Blackwater River, Minnesota",
            recordLengthDate: "October 20, 2001",
            
            // RESEARCH & SCIENTIFIC
            researchPriority: 10,
            geneticMarkers: "Extensive genetic studies for stock enhancement and conservation",
            studyPrograms: ["Minnesota DNR Muskellunge Research", "Great Lakes Fishery Commission", "Muskies Inc Research"],
            ...
            additionalMetadata: {
                ...
                "state_fish": "Wisconsin",
                "nickname": "Muskie"
            }
        )

        // Set storage paths using common name format
        self.VaultStoragePath = /storage/MuskellungeCoinVault
        self.VaultPublicPath = /public/MuskellungeCoinReceiver
        self.MinterStoragePath = /storage/MuskellungeCoinMinter
        self.MetadataAdminStoragePath = /storage/MuskellungeCoinMetadataAdmin
        self.FishDEXCoordinatorStoragePath = /storage/MuskellungeCoinFishDEXCoordinator
        self.FishDEXCoordinatorPublicPath = /public/MuskellungeCoinFishDEXCoordinator
        ...
        let cap = self.account.capabilities.storage.issue<&MuskellungeCoin.Vault>(self.VaultStoragePath)
