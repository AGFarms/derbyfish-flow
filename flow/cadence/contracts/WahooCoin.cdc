...
access(all) contract WahooCoin: FungibleToken {
    ...
    access(all) resource interface SpeciesCoinPublic {
        ...
        access(all) fun redeemCatchNFT(fishData: {String: AnyStruct}, angler: Address): @WahooCoin.Vault
    }
        ...
        switch viewType {
            ...
            case Type<FungibleTokenMetadataViews.FTVaultData>():
                return FungibleTokenMetadataViews.FTVaultData(
                    ...
                    receiverLinkedType: Type<&WahooCoin.Vault>(),
                    metadataLinkedType: Type<&WahooCoin.Vault>(),
                    createEmptyVaultFunction: (fun(): @{FungibleToken.Vault} {
                        return <-WahooCoin.createEmptyVault(vaultType: Type<@WahooCoin.Vault>())
                    })
                )
    ...
    access(all) resource FishDEXCoordinator: SpeciesCoinPublic {
        ...
        access(all) view fun getSpeciesCode(): String {
            return WahooCoin.speciesMetadata.speciesCode
        }
        
        access(all) view fun getTicker(): String {
            return WahooCoin.speciesMetadata.ticker
        }
        
        access(all) view fun getCommonName(): String {
            return WahooCoin.speciesMetadata.commonName
        }
        
        access(all) view fun getScientificName(): String {
            return WahooCoin.speciesMetadata.scientificName
        }
        
        access(all) view fun getFamily(): String {
            return WahooCoin.speciesMetadata.family
        }
        
        access(all) view fun getTotalSupply(): UFix64 {
            return WahooCoin.totalSupply
        }
        ...
        access(all) view fun getBasicRegistryInfo(): {String: AnyStruct} {
            return {
                ...
                "contractAddress": WahooCoin.account.address,
                "dataYear": WahooCoin.speciesMetadata.dataYear,
                "conservationStatus": WahooCoin.speciesMetadata.globalConservationStatus,
                "rarityTier": WahooCoin.speciesMetadata.rarityTier,
                "isRegistered": WahooCoin.isRegisteredWithFishDEX
            }
        }
        ...
        access(all) fun redeemCatchNFT(fishData: {String: AnyStruct}, angler: Address): @WahooCoin.Vault {
            ...
            if let nftId = fishNFTId {
                assert(
                    WahooCoin.fishNFTRegistry[nftId] == nil,
                    message: "This NFT has already been redeemed for a coin"
                )
                // Record this NFT as redeemed
                WahooCoin.fishNFTRegistry[nftId] = true
            }
            
            // Auto-record first catch if this is the very first mint
            if WahooCoin.totalSupply == 0.0 && WahooCoin.speciesMetadata.firstCatchDate == nil {
                WahooCoin.speciesMetadata.setFirstCatchDate(UInt64(getCurrentBlock().timestamp))
            ...
            WahooCoin.totalSupply = WahooCoin.totalSupply + amount
        ...
        access(all) fun registerWithFishDEX(fishDEXAddress: Address) {
            pre {
                !WahooCoin.isRegisteredWithFishDEX: "Already registered with FishDEX"
            }
            ...
            WahooCoin.fishDEXAddress = fishDEXAddress
            WahooCoin.isRegisteredWithFishDEX = true
        ...
        access(all) fun updateFishDEXAddress(newAddress: Address) {
            let oldAddress = WahooCoin.fishDEXAddress
            WahooCoin.fishDEXAddress = newAddress
            WahooCoin.isRegisteredWithFishDEX = false // Reset registration status
    ...
    access(all) resource Vault: FungibleToken.Vault {
        ...
        access(contract) fun burnCallback() {
            if self.balance > 0.0 {
                WahooCoin.totalSupply = WahooCoin.totalSupply - self.balance
        ...
        access(all) view fun getViews(): [Type] {
            return WahooCoin.getContractViews(resourceType: nil)
        }

        access(all) fun resolveView(_ view: Type): AnyStruct? {
            return WahooCoin.resolveContractView(resourceType: nil, viewType: view)
        }

        access(all) view fun getSupportedVaultTypes(): {Type: Bool} {
            return {Type<@WahooCoin.Vault>(): true}
        }

        access(all) view fun isSupportedVaultType(type: Type): Bool {
            return type == Type<@WahooCoin.Vault>()
        }
        ...
        access(FungibleToken.Withdraw) fun withdraw(amount: UFix64): @WahooCoin.Vault {
            self.balance = self.balance - amount
            return <-create Vault(balance: amount)
        }

        access(all) fun deposit(from: @{FungibleToken.Vault}) {
            let vault <- from as! @WahooCoin.Vault
            self.balance = self.balance + vault.balance
            vault.balance = 0.0
            destroy vault
        }

        access(all) fun createEmptyVault(): @WahooCoin.Vault {
    ...
    access(all) resource Minter {
        ...
        access(all) fun mintForCatch(amount: UFix64, fishId: UInt64, angler: Address): @WahooCoin.Vault {
            pre {
                amount == 1.0: "Only 1 coin per verified catch"
                WahooCoin.fishNFTRegistry[fishId] == nil: "This NFT has already been used to mint a coin"
            }
            
            // Record this NFT as used
            WahooCoin.fishNFTRegistry[fishId] = true
        ...
        access(all) fun autoRegisterWithFishDEX() {
            if let fishDEXAddr = WahooCoin.fishDEXAddress {
                if !WahooCoin.isRegisteredWithFishDEX {
                    // Get reference to FishDEX coordinator
                    if let coordinatorRef = WahooCoin.account.capabilities.borrow<&FishDEXCoordinator>(
                        WahooCoin.FishDEXCoordinatorPublicPath
                    ) {
        ...
        access(all) fun mintBatch(recipients: {Address: UFix64}): @{Address: WahooCoin.Vault} {
            let vaults: @{Address: WahooCoin.Vault} <- {}
    ...
    access(all) resource MetadataAdmin {
        ...
        access(all) fun updateImageURL(newURL: String) {
            let oldURL = WahooCoin.speciesMetadata.imageURL ?? ""
            WahooCoin.speciesMetadata.setImageURL(newURL)
        ...
        access(all) fun updateDescription(newDescription: String) {
            let oldDescription = WahooCoin.speciesMetadata.description
            WahooCoin.speciesMetadata.setDescription(newDescription)
        ...
        access(all) fun updateHabitat(newHabitat: String) {
            let oldHabitat = WahooCoin.speciesMetadata.habitat ?? ""
            WahooCoin.speciesMetadata.setHabitat(newHabitat)
        ...
        access(all) fun updateAverageWeight(newWeight: UFix64) {
            let oldWeight = WahooCoin.speciesMetadata.averageWeight?.toString() ?? "0.0"
            WahooCoin.speciesMetadata.setAverageWeight(newWeight)
        ...
        access(all) fun updateAverageLength(newLength: UFix64) {
            let oldLength = WahooCoin.speciesMetadata.averageLength?.toString() ?? "0.0"
            WahooCoin.speciesMetadata.setAverageLength(newLength)
        ...
        access(all) fun updateRarityTier(newTier: UInt8) {
            ...
            let oldTier = WahooCoin.speciesMetadata.rarityTier?.toString() ?? "0"
            WahooCoin.speciesMetadata.setRarityTier(newTier)
        ...
        access(all) fun manuallySetFirstCatch(timestamp: UInt64, angler: Address) {
            pre {
                WahooCoin.speciesMetadata.firstCatchDate == nil: "First catch already recorded"
            }
            WahooCoin.speciesMetadata.setFirstCatchDate(timestamp)
        ...
        access(all) fun updateConservationStatus(newStatus: String) {
            let oldStatus = WahooCoin.speciesMetadata.globalConservationStatus ?? ""
            WahooCoin.speciesMetadata.setConservationStatus(newStatus)
        ...
        access(all) fun updateSeasonalPatterns(newPatterns: String) {
            let oldPatterns = WahooCoin.speciesMetadata.seasonalPatterns ?? ""
            WahooCoin.speciesMetadata.setSeasonalPatterns(newPatterns)
        ...
        access(all) fun updateRecordWeight(newRecord: UFix64) {
            let oldRecord = WahooCoin.speciesMetadata.recordWeight?.toString() ?? "0.0"
            WahooCoin.speciesMetadata.setRecordWeight(newRecord)
        ...
        access(all) fun updateRecordLength(newRecord: UFix64) {
            let oldRecord = WahooCoin.speciesMetadata.recordLength?.toString() ?? "0.0"
            WahooCoin.speciesMetadata.setRecordLength(newRecord)
        ...
        access(all) fun updateRecordWeightLocation(newLocation: String?) {
            let oldLocation = WahooCoin.speciesMetadata.recordWeightLocation ?? ""
            WahooCoin.speciesMetadata.setRecordWeightLocation(newLocation)
        ...
        access(all) fun updateRecordWeightDate(newDate: String?) {
            let oldDate = WahooCoin.speciesMetadata.recordWeightDate ?? ""
            WahooCoin.speciesMetadata.setRecordWeightDate(newDate)
        ...
        access(all) fun updateRecordLengthLocation(newLocation: String?) {
            let oldLocation = WahooCoin.speciesMetadata.recordLengthLocation ?? ""
            WahooCoin.speciesMetadata.setRecordLengthLocation(newLocation)
        ...
        access(all) fun updateRecordLengthDate(newDate: String?) {
            let oldDate = WahooCoin.speciesMetadata.recordLengthDate ?? ""
            WahooCoin.speciesMetadata.setRecordLengthDate(newDate)
        ...
        access(all) fun approvePendingUpdate(index: Int) {
            pre {
                index >= 0 && index < WahooCoin.pendingUpdates.length: "Invalid update index"
            }
            let update = WahooCoin.pendingUpdates.remove(at: index)
        ...
        access(all) fun updateConservationStatusValidated(newStatus: String) {
            pre {
                WahooCoin.validateConservationStatus(newStatus): "Invalid conservation status"
            }
            let oldStatus = WahooCoin.speciesMetadata.globalConservationStatus ?? ""
            WahooCoin.speciesMetadata.setConservationStatus(newStatus)
            emit MetadataUpdated(field: "globalConservationStatus", oldValue: oldStatus, newValue: newStatus)
        ...
        access(all) fun updateFightRatingValidated(newRating: UInt8) {
            pre {
                WahooCoin.validateRating(newRating): "Fight rating must be 1-10"
            }
            let oldRating = WahooCoin.speciesMetadata.fightRating?.toString() ?? "0"
            WahooCoin.speciesMetadata.setFightRating(newRating)
        ...
        access(all) fun archiveCurrentYear() {
            let currentYear = WahooCoin.currentMetadataYear
            WahooCoin.metadataHistory[currentYear] = WahooCoin.speciesMetadata
            emit YearlyMetadataCreated(year: currentYear)
        }
        
        access(all) fun updateToNewYear(_ newYear: UInt64) {
            pre {
                newYear > WahooCoin.currentMetadataYear: "New year must be greater than current year"
            }
            // Archive current year data
            self.archiveCurrentYear()
            
            let oldYear = WahooCoin.currentMetadataYear
            WahooCoin.currentMetadataYear = newYear
            emit MetadataYearUpdated(oldYear: oldYear, newYear: newYear)
        }
    }

    // Public functions
    access(all) fun createEmptyVault(vaultType: Type): @WahooCoin.Vault {
        pre {
            vaultType == Type<@WahooCoin.Vault>(): "Vault type mismatch"
        }
        return <-create Vault(balance: 0.0)
    }
    ...
    // NOTE: Wahoo coins represent permanent historical catch records
    ...
    access(all) view fun getVaultBalance(vaultRef: &WahooCoin.Vault): UFix64 {
        return vaultRef.balance
    }
    
    access(all) view fun canWithdraw(vaultRef: &WahooCoin.Vault, amount: UFix64): Bool {
    ...
    access(all) view fun isValidVaultType(vaultType: Type): Bool {
        return vaultType == Type<@WahooCoin.Vault>()
    }
    ...
    init() {
        ...
        // Set comprehensive species metadata for Wahoo
        self.speciesMetadata = SpeciesMetadata(
            // IMMUTABLE CORE FIELDS
            speciesCode: "ACANTHOCYBIUM_SOLANDRI",
            ticker: "ACASOL",
            ...
            // BASIC DESCRIPTIVE FIELDS
            commonName: "Wahoo",
            habitat: "Tropical and subtropical pelagic waters",
            averageWeight: 30.0,
            averageLength: 48.0,
            imageURL: "https://derby.fish/images/species/Wahoo.jpg",
            description: "Wahoo are high-speed pelagic predators known for their incredible speed (up to 60 mph) and excellent eating quality. They have distinctive blue-green stripes and razor-sharp teeth, making them one of the most sought-after gamefish in offshore waters.",
            firstCatchDate: nil,
            rarityTier: 4,
            ...
            regionalPopulations: {
                "Atlantic": RegionalPopulation(
                    populationTrend: "Stable",
                    threats: ["Overfishing", "Climate Change", "Habitat Loss"],
                    protectedAreas: ["Marine Protected Areas"],
                    estimatedPopulation: nil
                ),
                "Pacific": RegionalPopulation(
                    populationTrend: "Stable",
                    threats: ["Commercial Fishing", "Bycatch", "Ocean Warming"],
                    protectedAreas: ["Pacific Remote Islands Marine National Monument"],
                    estimatedPopulation: nil
                ),
                "Caribbean": RegionalPopulation(
                    populationTrend: "Stable",
                    threats: ["Overfishing", "Coastal Development", "Pollution"],
            ...
            // BIOLOGICAL INTELLIGENCE
            lifespan: 9.0,
            diet: "Flying fish, squid, mackerel, herring, pilchards, scad",
            predators: ["Large Sharks", "Marlin", "Tuna", "Humans"],
            temperatureRange: "68-86°F (optimal 75-82°F)",
            depthRange: "0-250 feet (surface to mid-water)",
            spawningAge: 2.0,
            spawningBehavior: "Spawns in warm waters year-round, releases pelagic eggs in open ocean",
            migrationPattern: "Highly migratory, follows warm currents and baitfish schools",
            waterQualityNeeds: "Clear, warm saltwater with high dissolved oxygen",
            
            // GEOGRAPHIC & HABITAT
            nativeRegions: ["Tropical Atlantic", "Pacific Ocean", "Indian Ocean"],
            currentRange: ["All tropical and subtropical oceans worldwide"],
            waterTypes: ["Open Ocean", "Pelagic", "Continental Shelf", "Blue Water"],
            ...
            tourismValue: 10,
            ecosystemRole: "Apex Predator (pelagic waters), Key Predator",
            culturalSignificance: "Highly prized by sport fishing communities worldwide, important to island economies",
            
            // ANGLING & RECREATIONAL
            bestBaits: ["High-speed lures", "Ballyhoo", "Flying fish", "Squid", "Live bait"],
            fightRating: 9,
            culinaryRating: 9,
            catchDifficulty: 8,
            seasonalAvailability: "Year-round in tropical waters, seasonal in temperate zones",
            bestTechniques: ["High-speed trolling", "Live bait fishing", "Kite fishing", "Chumming"],
            ...
            regionalRegulations: {
                "Florida": RegionalRegulations(
                    ...
                    bagLimit: 2,
                    closedSeasons: [],
                    specialRegulations: "No minimum size limit, bag limit applies",
                ...
                "Hawaii": RegionalRegulations(
                    ...
                    bagLimit: nil,
                    closedSeasons: [],
                    specialRegulations: "No specific regulations",
                    licenseRequired: true
                ),
                "Caribbean": RegionalRegulations(
                    ...
                    bagLimit: nil,
                    closedSeasons: [],
                    specialRegulations: "Varies by country/territory",
                    licenseRequired: true
                )
            },
            
            // PHYSICAL & BEHAVIORAL
            physicalDescription: "Elongated body with blue-green back, silver sides with vertical blue stripes, large mouth with razor-sharp teeth",
            behaviorTraits: "Extremely fast swimmer, solitary or small groups, ambush predator, surface to mid-water feeder",
            seasonalPatterns: "Follows warm water masses, more active during dawn and dusk, year-round spawning",
            
            // RECORDS & ACHIEVEMENTS
            recordWeight: 184.0,
            recordWeightLocation: "Cabo San Lucas, Mexico",
            recordWeightDate: "July 15, 2005",
            recordLength: 96.0,
            recordLengthLocation: "Bahamas",
            recordLengthDate: "March 22, 1990",
            
            // RESEARCH & SCIENTIFIC
            researchPriority: 7,
            geneticMarkers: "Population genetics studies for stock assessment",
            studyPrograms: ["NOAA Fisheries Research", "Pacific Fisheries Research", "Caribbean Fisheries Studies"],
        ...
        self.VaultStoragePath = /storage/WahooCoinVault
        self.VaultPublicPath = /public/WahooCoinReceiver
        self.MinterStoragePath = /storage/WahooCoinMinter
        self.MetadataAdminStoragePath = /storage/WahooCoinMetadataAdmin
        self.FishDEXCoordinatorStoragePath = /storage/WahooCoinFishDEXCoordinator
        self.FishDEXCoordinatorPublicPath = /public/WahooCoinFishDEXCoordinator
        ...
        let cap = self.account.capabilities.storage.issue<&WahooCoin.Vault>(self.VaultStoragePath)
